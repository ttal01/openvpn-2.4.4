...
void
platform_nice(int niceval)//改变程序优先级
{
    if (niceval)
    {
#ifdef HAVE_NICE
...
/* Disable paging */
void
platform_mlockall(bool print_msg)//输出相关信息
{
#ifdef HAVE_MLOCKALL
...
int
platform_access(const char *path, int mode)//检查程序对文件的权限
{
#ifdef _WIN32
    struct gc_arena gc = gc_new();
...
FILE *
platform_fopen(const char *path, const char *mode)//打开文件
{
#ifdef _WIN32
    struct gc_arena gc = gc_new();
    FILE *f = _wfopen(wide_string(path, &gc), wide_string(mode, &gc));  //???为何不用标准库函数fopen()
    gc_free(&gc);
    return f;
#else
...
int
platform_open(const char *path, int flags, int mode)//调用open（）
{
#ifdef _WIN32
    struct gc_arena gc = gc_new();
...
int
platform_stat(const char *path, platform_stat_t *buf)//获取文件信息 存入参数二指向的内存中
{
#ifdef _WIN32
    struct gc_arena gc = gc_new();
