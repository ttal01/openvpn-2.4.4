...
int
dev_type_enum(const char *dev, const char *dev_type)//根据设备名称确定通道类型
{
...
guess_tuntap_dev(const char *dev,
                 const char *dev_type,
                 const char *dev_node,
                 struct gc_arena *gc)//在设备实际打开之前，尝试预测实际的TUN/TAP设备实例名
{
#ifdef _WIN32
    const int dt = dev_type_enum(dev, dev_type);//根据设备名称确定通道类型
 ...
 */
bool
is_tun_p2p(const struct tuntap *tt)//点对点拓扑结构返回1
{
    bool tun = false;
 ...
 */
void
do_ifconfig_setenv(const struct tuntap *tt, struct env_set *es)//设置ifconfig_*环境变量
{
    struct gc_arena gc = gc_new();
...
init_tun(const char *dev,        /* --dev option */
         ...
         struct env_set *es)//初始化tun/tap对象
{
    struct gc_arena gc = gc_new();
    struct tuntap *tt;
...
init_tun_post(struct tuntap *tt,
              ...
              const struct tuntap_options *options)//参数一初始化
{
    tt->options = *options;
#ifdef _WIN32
...
do_ifconfig(struct tuntap *tt,
            ...
            int tun_mtu,
            const struct env_set *es)//通过shell执行ifconfig命令

{
    ...
    if (tt->did_ifconfig_setup)
    {
         ...
         * We only handle TUN/TAP devices here, not --dev null devices.
         */
        tun = is_tun_p2p(tt);//点对点拓扑结构返回1

        /*
         * Set ifconfig parameters
         */
        ifconfig_local = print_in_addr_t(tt->local, 0, &gc);//网络字节序转为主机字节序
        ifconfig_remote_netmask = print_in_addr_t(tt->remote_netmask, 0, &gc);//网络字节序转为主机字节序
        ...
        if (!tun)
        {
            ifconfig_broadcast = print_in_addr_t(tt->broadcast, 0, &gc);//网络字节序转为主机字节序
        }
            ...
            switch (tt->options.ip_win32_type)
            {
                case IPW32_SET_MANUAL:
                    msg(M_INFO, "******** NOTE:  Please manually set the IP/netmask of '%s' to %s/%s (if it is not already set)",
                        actual,
                        ifconfig_local,
                        print_in_addr_t(tt->adapter_netmask, 0, &gc));//网络字节序转为主机字节序
                    break;
...
get_adapter_info_list(struct gc_arena *gc)//得到适配器信息列表
{
    ULONG size = 0;
    ...
    if ((status = GetAdaptersInfo(NULL, &size)) != ERROR_BUFFER_OVERFLOW)
    {
        msg(M_INFO, "GetAdaptersInfo #1 failed (status=%u) : %s",
            (unsigned int)status,
            strerror_win32(status, gc));//返回Windows错误信息
    }
    else
    {
        pi = (PIP_ADAPTER_INFO) gc_malloc(size, false, gc);//分配空间 创建并关联垃圾回收条目 并返回数据区偏移地址
