...
sf2gaf(const unsigned int getaddr_flags,
       const unsigned int sockflags)//转换标志位
{
...
getaddr(unsigned int flags,
        ...
        bool *succeeded,
        volatile int *signal_received)//将IPv4/IPv6 addr or hostname转为网络字节序
{
    struct addrinfo *ai;
    int status;
    status = openvpn_getaddrinfo(flags & ~GETADDR_HOST_ORDER, hostname, NULL,
                                 resolve_retry_seconds, signal_received, AF_INET, &ai);//将IPv4/IPv6 addr or hostname解析并保存到末尾参数中
    if (status==0)
    {
        struct in_addr ia;
        if (succeeded)
...
static inline bool
streqnull(const char *a, const char *b)//比较两个字符串 相等或都为空时返回1
{
    if (a == NULL && b == NULL)
    {
        return true;
    ...
    {
        return streq(a, b);//比较两个字符串 相等返回1
    }
}
...
get_cached_dns_entry(struct cached_dns_entry *dns_cache,
                     ...
                     int ai_family,
                     int resolve_flags,
                     struct addrinfo **ai)//从现有链表中获取存储的主机名、服务器名 存入末尾参数中 匹配返回0
{
    ...
    {
        if (streqnull(ph->hostname, hostname)//比较两个字符串 相等或都为空时返回1
...
do_preresolve_host(struct context *c,
                   ...
                   const char *servname,
                   const int af,
                   const int flags)//确保主机名、服务器名存入现有参数一的c1.dns_cache成员中 成功返回0
{
    struct addrinfo *ai;
    int status;
    ...
    if (get_cached_dns_entry(c->c1.dns_cache,
                             ...
                             af,
                             flags,
                             &ai) == 0)//从现有链表中获取存储的主机名、服务器名 存入末尾参数中 匹配返回0
    {
    ...
    status = openvpn_getaddrinfo(flags, hostname, servname,
                                 ...
                                 af, &ai);//将IPv4/IPv6 addr or hostname解析并保存到末尾参数中 成功返回0
    if (status == 0)//成功
    {
        struct cached_dns_entry *ph;

        ALLOC_OBJ_CLEAR_GC(ph, struct cached_dns_entry, &c->gc);//分配空间 创建并关联垃圾回收条目 并返回数据区偏移地址
        ph->ai = ai;
        ...
        gc_addspecial(ai, &gc_freeaddrinfo_callback, &c->gc);//将参数一参数二存入堆空间并插入参数三的链表中
...
void
do_preresolve(struct context *c)//确保主机名、服务器名存入现有参数的c1.dns_cache成员中 负责设定软中断信号
{
    int i;
    ...
    for (i = 0; i < l->len; ++i)
    {
        ...
        if (proto_is_dgram(ce->proto))//判断是否为udp协议 是则为1
        {
        ...
        {
            status = do_preresolve_host(c, remote, ce->remote_port, ce->af, flags);//确保主机名、服务器名存入现有参数一的c1.dns_cache成员中 成功返回0
            if (status != 0)
            {
                goto err;
            }
        ...
        if (ce->http_proxy_options)
        {
            status = do_preresolve_host(c,
                                        ...
                                        ce->af,
                                        preresolve_flags);//确保主机名、服务器名存入现有参数一的c1.dns_cache成员中 成功返回0
        ...
        if (ce->socks_proxy_server)
        {
            status = do_preresolve_host(c,
                                        ...
                                        ce->socks_proxy_port,
                                        ce->af,
                                        flags);//确保主机名、服务器名存入现有参数一的c1.dns_cache成员中 成功返回0
            if (status != 0)
            {
                goto err;
            }
        ...
        if (ce->bind_local)
        {
            ...
            flags &= ~GETADDR_RANDOMIZE;
            status = do_preresolve_host(c, ce->local, ce->local_port, ce->af, flags);//确保主机名、服务器名存入现有参数一的c1.dns_cache成员中 成功返回0
            if (status != 0)
            {
                goto err;
            }
...
err:
    throw_signal_soft(SIGHUP, "Preresolving failed");//信号软中断
}
...
openvpn_getaddrinfo(unsigned int flags,
                    ...
                    int ai_family,
                    struct addrinfo **res)//将IPv4/IPv6 addr or hostname解析并保存到末尾参数中 成功返回0
{
    ...
    status = getaddrinfo(hostname, servname, &hints, res);//库函数 处理名字到地址以及服务到端口这两种转换 存储到末尾参数
    ...
    if (status != 0) /* parse as numeric address failed? */
    {
        ...
        {
            hostname = hostname_randomize(hostname, &gc);//相关打印信息
        }
...
static bool
socket_set_tcp_nodelay(int sd, int state)//设置其它的套接字选项
{
...
socket_set_flags(int sd, unsigned int sockflags)
{
    ...
    {
        return socket_set_tcp_nodelay(sd, 1);//设置其它的套接字选项
    }
    else
    {
        return true;
...
link_socket_update_flags(struct link_socket *ls, unsigned int sockflags)//???
{
    ...
    {
        return socket_set_flags(ls->sd, ls->sockflags = sockflags);//设置其它的套接字选项
    }
    else
    {
        return false;
...
socket_descriptor_t
create_socket_tcp(struct addrinfo *addrinfo)//创建tcp套接字 端口复用
{
    ...
    if ((sd = socket(addrinfo->ai_family, addrinfo->ai_socktype, addrinfo->ai_protocol)) < 0)//创建套接字
    {
...
socket_do_accept(socket_descriptor_t sd,
                 ...
                 const bool nowait)//接收inet连接请求 设置文件描述符fork时关闭 返回连接描述符
{
     ...
     */
    socklen_t remote_len_af = af_addr_size(act->dest.addr.sa.sa_family);//返回预连接的套接字大小
    ...
    else if (remote_len_af && remote_len != remote_len_af)//连接的套接字大小与计划的大小不一致时
    {
            ...
            bool ipv6only)//绑定套接字地址
{
    ...
    if (bind(sd, cur->ai_addr, cur->ai_addrlen))//绑定套接字地址
    {
        msg(M_FATAL | M_ERRNO, "%s: Socket bind failed on local address %s",
            prefix,
            print_sockaddr_ex(local->ai_addr, ":", PS_SHOW_PORT, &gc));//根据套接字地址获取主机及服务名 按格式保存到局部缓冲区中 并返回地址
    }
    gc_free(&gc);
}
...
openvpn_connect(socket_descriptor_t sd,
                ...
                int connect_timeout,
                volatile int *signal_received)//连接connect()
{
    int status = 0;
...
void
set_actual_address(struct link_socket_actual *actual, struct addrinfo *ai)//根据参数二设定参数一
{
    CLEAR(*actual);
    ASSERT(ai);
...
static void
resolve_bind_local(struct link_socket *sock, const sa_family_t af)//解析参数一所含信息
{
    struct gc_arena gc = gc_new();//返回一个已初始化好的gc_arena临时变量 用于给外部变量赋值 没有分配内存
    ...
    if (!sock->info.lsa->bind_local)
    {
        ...
        if (proto_is_dgram(sock->info.proto))//判断是否为udp协议
        {
            flags |= GETADDR_DATAGRAM;
        }
        ...
        status = get_cached_dns_entry(sock->dns_cache,
                                      ...
                                      af,
                                      flags,
                                      &sock->info.lsa->bind_local);//从现有链表中获取存储的主机名、服务器名 存入末尾参数中 匹配返回0

        if (status)//失败
        {
            status = openvpn_getaddrinfo(flags, sock->local_host, sock->local_port, 0,
                                         NULL, af, &sock->info.lsa->bind_local);//将IPv4/IPv6 addr or hostname保存到末尾参数中
        }
...
resolve_remote(struct link_socket *sock,
               int phase,
               const char **remote_dynamic,
               volatile int *signal_received)//解析参数一
{
    struct gc_arena gc = gc_new();//返回一个已初始化好的gc_arena临时变量 用于给外部变量赋值 没有分配内存

    /* resolve remote address if undefined */
    if (!sock->info.lsa->remote_list)//没有定义的话
    {
        if (sock->remote_host)
        {
            unsigned int flags = sf2gaf(GETADDR_RESOLVE|GETADDR_UPDATE_MANAGEMENT_STATE, sock->sockflags);//转换标志位
            int retry = 0;
            int status = -1;
            struct addrinfo *ai;
            if (proto_is_dgram(sock->info.proto))//判断是否为udp协议
            ...
            status = get_cached_dns_entry(sock->dns_cache,
                                          ...
                                          sock->remote_port,
                                          sock->info.af,
                                          flags, &ai);//从现有链表中获取存储的主机名、服务器名 存入末尾参数中 匹配返回0
            if (status)
            {
                status = openvpn_getaddrinfo(flags, sock->remote_host, sock->remote_port,
                                             retry, signal_received, sock->info.af, &ai);//将IPv4/IPv6 addr or hostname解析并保存到末尾参数中
            }
    ...
    if (link_socket_actual_defined(&sock->info.lsa->actual))//地址是否定义 是则返回1
    {
    ...
    else
    {
        ...
        if (sock->info.lsa->current_remote)
        {
            set_actual_address(&sock->info.lsa->actual,
                               sock->info.lsa->current_remote);//根据参数二设定参数一
        }
    }
...
struct link_socket *
link_socket_new(void)//分配空间并初始化
{
    struct link_socket *sock;

    ALLOC_OBJ_CLEAR(sock, struct link_socket);//分配空间并置空
...
link_socket_init_phase1(struct link_socket *sock,
                        ...
                        unsigned int sockflags)//socket初始化
{
    ASSERT(sock);
    ...
    else if (mode != LS_MODE_TCP_ACCEPT_FROM)
    {
        if (sock->bind_local)
        {
            resolve_bind_local(sock, sock->info.af);//解析参数一所含信息
        }
        resolve_remote(sock, 1, NULL, NULL);//解析参数一
    }
}
...
{
    const char *host = print_sockaddr_ex(&info->lsa->actual.dest.addr.sa, " ", PS_SHOW_PORT, gc);//根据套接字地址获取主机及服务名 按格式保存到局部缓冲区中 并返回地址
    if (include_cmd)
    {
    ...
    switch (from_addr->dest.addr.sa.sa_family)
    {
        ...
        case AF_INET6:
            msg(D_LINK_ERRORS,
                ...
                print_sockaddr_ex(info->lsa->remote_list->ai_addr,":",PS_SHOW_PORT, &gc));//根据套接字地址获取主机及服务名 按格式保存到局部缓冲区中 并返回地址
            ...
            for (ai = info->lsa->remote_list->ai_next; ai; ai = ai->ai_next)
            {
                msg(D_LINK_ERRORS,"or from peer address: %s",
                    print_sockaddr_ex(ai->ai_addr,":",PS_SHOW_PORT, &gc));//根据套接字地址获取主机及服务名 按格式保存到局部缓冲区中 并返回地址
            }
            break;
    }
    buf->len = 0;
...
print_sockaddr_ex(const struct sockaddr *sa,
                  ...
                  struct gc_arena *gc)//根据套接字地址获取主机及服务名 按格式保存到局部缓冲区中 并返回地址
{
    struct buffer out = alloc_buf_gc(128, gc);//返回局部缓冲区变量 内含已注册关联的堆空间信息
    ...
    switch (sa->sa_family)
    {
        case AF_INET:
            ...
            {
                buf_puts(&out, "[AF_INET]");//拷贝参数二信息到参数一的缓冲区成员中
            }
    ...
    //系统函数 以一个套接口地址为参数，返回一个描述主机的字符串和一个描述服务的字符串
    status = getnameinfo(sa, salen, hostaddr, sizeof(hostaddr),
                         servname, sizeof(servname), NI_NUMERICHOST | NI_NUMERICSERV);

    if (status!=0)
    {
        buf_printf(&out,"[nameinfo() err: %s]",gai_strerror(status));//格式化输出（附加）到缓冲区中
        return BSTR(&out);
    }
    ...
    if (!(flags & PS_DONT_SHOW_ADDR))
    {
        if (addr_is_defined)
        {
            buf_puts(&out, hostaddr);//拷贝参数二信息到参数一的缓冲区成员中
        }
        else
        {
            buf_puts(&out, "[undef]");//拷贝参数二信息到参数一的缓冲区成员中
        }
    }
    ...
    if ((flags & PS_SHOW_PORT) || (flags & PS_SHOW_PORT_IF_DEFINED))
    {
        if (separator)
        {
            buf_puts(&out, separator);//拷贝参数二信息到参数一的缓冲区成员中
        }

        buf_puts(&out, servname);//拷贝参数二信息到参数一的缓冲区成员中
    }

    return BSTR(&out);//返回当前缓冲区的内部上一条数据结尾偏移地址（即当前数据开始地址）
}
 ...
 */
const char *
print_in_addr_t(in_addr_t addr, unsigned int flags, struct gc_arena *gc)//网络字节序转为主机字节序
{
    struct in_addr ia;
    struct buffer out = alloc_buf_gc(64, gc);//返回局部缓冲区变量 内含已注册关联的堆空间信息
    ...
    if (addr || !(flags & IA_EMPTY_IF_UNDEF))
    {
        ...
        buf_printf(&out, "%s", inet_ntoa(ia));//网络字节序转为主机字节序
    }
    return BSTR(&out);
}
...
bool
proto_is_net(int proto)//判断协议是否定义
{
...
bool
proto_is_dgram(int proto)//判断是否为udp协议 是返回1
{
    return proto_is_udp(proto);
}

bool
proto_is_udp(int proto)//判断是否为udp协议
{
    if (proto < 0 || proto >= PROTO_N)
    {
        ASSERT(0);
...
const char *
proto2ascii(int proto, sa_family_t af, bool display_form)//返回协议对应的说明
{
    unsigned int i;
    for (i = 0; i < SIZE(proto_names); ++i)
    {
...
link_socket_read_udp_posix(struct link_socket *sock,
                           struct buffer *buf,
                           struct link_socket_actual *from)//从套接字中读取数据到参数二
{
    socklen_t fromlen = sizeof(from->dest.addr);
...
/*
 * Win32 overlapped socket I/O functions.  Win32重叠了I/O函数
 */
...
/*
 * Socket event notification 套接字事件通知
 */
...
socket_set(struct link_socket *s,
           ...
           void *arg,
           unsigned int *persistent)//套接字事件状态控制设置
{
    if (s)
    {
        ...
        /* if persistent is defined, call event_ctl only if rwflags has changed since last call */
                //如果已经定义了持久性，那么仅当自上次调用时rwflags更改后 才调用event_ctl
        if (!persistent || *persistent != rwflags)
        {
            event_ctl(es, socket_event_handle(s), rwflags, arg);//调用事件修改模式 调整事件可读可写监测状态
            if (persistent)
            {
                *persistent = rwflags;
            }
...
void
sd_close(socket_descriptor_t *sd)//关闭套接字
{
    if (sd && socket_defined(*sd))
    {
        openvpn_close_socket(*sd);//close(fd)
        *sd = SOCKET_UNDEFINED;
    }
}
...
const char *
sockaddr_unix_name(const struct sockaddr_un *local, const char *null)//返回绑定的unix地址路径
{
...
socket_descriptor_t
create_socket_unix(void)//创建并返回套接字
{
    socket_descriptor_t sd;
...
socket_bind_unix(socket_descriptor_t sd,
                 ...
                 const char *prefix)//更改权限以绑定套接字地址
{
    struct gc_arena gc = gc_new();

#ifdef HAVE_UMASK
    const mode_t orig_umask = umask(0);//设置掩码为000，文件默认777，返回原mask
#endif

    if (bind(sd, (struct sockaddr *) local, sizeof(struct sockaddr_un)))//绑定套接字地址
    {
        msg(M_FATAL | M_ERRNO,
...
socket_accept_unix(socket_descriptor_t sd,
                   struct sockaddr_un *remote)//接收unix连接请求 设置文件描述符fork时关闭 返回连接描述符
{
    socklen_t remote_len = sizeof(struct sockaddr_un);
    socket_descriptor_t ret;

    CLEAR(*remote);
    ret = accept(sd, (struct sockaddr *) remote, &remote_len);//接收连接请求
    if (ret >= 0)
    {
        /* set socket file descriptor to not pass across execs, so that
         * scripts don't have access to it */
        set_cloexec(ret);//设置文件描述符fork时关闭
    }
    return ret;
}
...
socket_connect_unix(socket_descriptor_t sd,
                    struct sockaddr_un *remote)//连接connect()
{
    int status = connect(sd, (struct sockaddr *) remote, sizeof(struct sockaddr_un));//连接connect()
    if (status)
    {
        status = openvpn_errno();
    }
...
void
socket_delete_unix(const struct sockaddr_un *local)//删除unix路径指定的硬链接或文件
{
    const char *name = sockaddr_unix_name(local, NULL);//返回绑定的unix地址路径
#ifdef HAVE_UNLINK
    if (name && strlen(name))
    {
        unlink(name);//删除一个硬链接或文件
    }
#endif
}

bool
unix_socket_get_peer_uid_gid(const socket_descriptor_t sd, int *uid, int *gid)//获取uid gid
{
    ...
    struct ucred peercred;  //上一行，实际在linux中是定义的
