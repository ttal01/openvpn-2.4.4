...
static inline bool
buf_defined(const struct buffer *buf)   //判断存在成功分配的堆空间
{
    return buf->data != NULL;
}

static inline bool
buf_valid(const struct buffer *buf) //判断存在成功分配的堆空间 且长度不为负数
{
    return likely(buf->data != NULL) && likely(buf->len >= 0);
}

static inline uint8_t *
buf_bptr(const struct buffer *buf)  //返回当前缓冲区的内部上一条数据结尾偏移地址（即当前数据开始地址）
{
    if (buf_valid(buf))//判断存在成功分配的堆空间 且长度不为负数
    {
...
static int
buf_len(const struct buffer *buf)   //返回当前缓冲区的内部数据长度
{
    if (buf_valid(buf))
    {
        return buf->len;
...
static inline uint8_t *
buf_bend(const struct buffer *buf)  //返回当前当前缓冲区数据结尾地址
{
    return buf_bptr(buf) + buf_len(buf);    //返回 当前缓冲区的内部偏移地址 + 当前缓冲区的内部数据长度
}

static inline uint8_t *
buf_blast(const struct buffer *buf)//返回结尾字符地址
{
    if (buf_len(buf) > 0)//返回当前缓冲区的内部数据长度
    {
...
static inline bool
buf_size_valid(const size_t size)//判断分配空间大小范围是否有效
{
    return likely(size < BUF_SIZE_MAX);
}

static inline bool
buf_size_valid_signed(const int size)//参数在最大缓冲区大小正负值之间 返回1
{
    return likely(size >= -BUF_SIZE_MAX) && likely(size < BUF_SIZE_MAX);
}

static inline char *
buf_str(const struct buffer *buf)//返回当前缓冲区的内部上一条数据结尾偏移地址（即当前数据开始地址）
{
    return (char *)buf_bptr(buf);
}

static inline void
buf_reset(struct buffer *buf)//重置参数成员值
{
    buf->capacity = 0;
    buf->offset = 0;
    buf->len = 0;
...
static inline bool
buf_init_dowork(struct buffer *buf, int offset)//初始化参数一
{
 ...
 */
static inline void
secure_memzero(void *data, size_t len)//置空参数一空间
{
#if defined(_WIN32)
...
static inline bool
buf_safe(const struct buffer *buf, int len)//检查是否有足够的空间追加缓冲区。
{
...
static inline bool
buf_safe_bidir(const struct buffer *buf, int len)//确定参数一变更参数二长度后仍然有效
{
    if (buf_valid(buf) && buf_size_valid_signed(len))//参数在最大缓冲区大小正负值之间 返回1
    {
...
static inline int
buf_forward_capacity(const struct buffer *buf)  //返回当前缓冲区剩余空间大小
{
    if (buf_valid(buf))//判断存在成功分配的堆空间 且长度不为负数
    {
        int ret = buf->capacity - (buf->offset + buf->len);//缓冲区大小 - 当前缓冲区已用大小
        if (ret < 0)
        {
            ret = 0;
        }
...
static inline bool
buf_inc_len(struct buffer *buf, int inc)//参数一变更参数二长度
{
    if (!buf_safe_bidir(buf, inc))//确定参数一变更参数二长度后仍然有效
    {
        return false;
    }
    buf->len += inc;
...
/*
 * Make space to prepend to a buffer.留出空间以预制缓冲区。
...
buf_prepend(struct buffer *buf, int size)
{
    ...
    buf->offset -= size;//？？？
    buf->len += size;
    return BPTR(buf);//返回当前缓冲区的内部上一条数据结尾偏移地址（即当前数据开始地址）
}

static inline bool
buf_advance(struct buffer *buf, int size)//数据消除回退 用于按size大小记录读取的数据位置
{
...
static inline uint8_t *
buf_write_alloc(struct buffer *buf, int size)//返回一个在缓冲区内分配空间的指针 指向最后一条数据末尾
{
    uint8_t *ret;
    if (!buf_safe(buf, size))//检查是否有足够的空间追加缓冲区。
    {
        return NULL;
    }
    ret = BPTR(buf) + buf->len;//现有数据的末尾
    buf->len += size;//预先修改数据长度 后面会添加size长度的数据
    return ret;
}
...
static inline bool
buf_write(struct buffer *dest, const void *src, int size)//将参数三长度的参数二拷贝到参数一
{
    uint8_t *cp = buf_write_alloc(dest, size);//返回一个在缓冲区内分配空间的指针(最后一条数据的末尾)
    if (!cp)
    {
        return false;
    }
    memcpy(cp, src, size);//拷贝数据
    return true;
}
...
static inline bool
buf_write_u8(struct buffer *dest, int data)//添加数据到参数一
{
    uint8_t u8 = (uint8_t) data;
    return buf_write(dest, &u8, sizeof(uint8_t));//将参数三长度的参数二拷贝到参数一
}

static inline bool
buf_write_u16(struct buffer *dest, int data)//添加数据到参数一
{
    uint16_t u16 = htons((uint16_t) data);
    return buf_write(dest, &u16, sizeof(uint16_t));//将参数三长度的参数二拷贝到参数一
}
...
static inline bool
buf_copy(struct buffer *dest, const struct buffer *src)//拷贝参数二到参数一
{
...
buf_copy_excess(struct buffer *dest,
                struct buffer *src,
                int len)//将参数二截断，将多余的数据复制到参数一
{
    if (len < 0)
    {
        return false;
    }
    if (src->len > len)//缓冲区中 向后移动len长度
    {
        struct buffer b = *src;
        src->len = len;
        if (!buf_advance(&b, len))//数据消除回退 用于按size大小记录读取的数据位置
        {
            return false;
        }
        return buf_copy(dest, &b);//拷贝参数二到参数一
    }
    else
    {
        return true;
...
static inline int
buf_read_u8(struct buffer *buf)//返回预读空间起始地址 并跳到下一个字符
{
    int ret;
    if (BLEN(buf) < 1)//返回当前缓冲区的内部数据长度
    {
        return -1;
    }
    ret = *BPTR(buf);//返回当前缓冲区的内部上一条数据结尾偏移地址（即当前数据开始地址）
    buf_advance(buf, 1);//数据消除回退  用于按size大小记录读取的数据位置
    return ret;
}
...
/*
 * Verify that a pointer is correctly aligned 验证指针是否正确对齐
 */
#ifdef VERIFY_ALIGNMENT
...
gc_new(void)			//返回一个已初始化好的gc_arena临时变量 用于给外部变量赋值 没有分配内存
{
    struct gc_arena ret;
    gc_init(&ret);
    return ret;
...
static inline void
gc_free(struct gc_arena *a)//释放空间
{
    if (a->list)
    {
        x_gc_free(a);//释放空间
...
static inline void
check_malloc_return(const void *p)  //检查 若为空 则输出信息 并退出
{
    if (!p)
    {
        out_of_memory();
