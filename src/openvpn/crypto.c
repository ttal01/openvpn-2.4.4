 ...
 * 如果(opt ->flags & CO_USE_IV)不是NULL，我们将从包中读取IV。
 * Set buf->len to 0 and return false on decrypt error.
 * 将buf-len设置为0，并在解密错误中返回false。
 * On success, buf is set to point to plaintext, true
 * is returned. 成功,buf将指向明文，true被返回。
 */
static bool
 ...
 * 将buf-len设置为0，并在解密错误中返回false。
 * On success, buf is set to point to plaintext, true
 * is returned.  On success,buf将指向明文，true被返回。
 */
static bool
