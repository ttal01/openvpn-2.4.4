...
socks_proxy_new(const char *server,
                const char *port,
                const char *authfile)//分配空间 使用参数创建新的代理
{
    struct socks_proxy_info *p;

    ALLOC_OBJ_CLEAR(p, struct socks_proxy_info);//分配空间并置空
 ...
 * UDP packet, setting *from to the source address. 从传入的UDP包中删除10个字节的socks5头，将*from设置为从源地址。
 *
 * Run after UDP read.UDP阅读后运行。
 */
void
...
socks_process_outgoing_udp(struct buffer *buf,
                           const struct link_socket_actual *to)//在udp包头部添加10字节头信息 返回10
{
    /*
     ...
     */
    struct buffer head = buf_sub(buf, 10, true);//返回一个用于写入的缓冲区，它是另一个缓冲区的子集

    /* crash if not enough headroom in buf */
    ASSERT(buf_defined(&head));//判断存在成功分配的堆空间

    buf_write_u16(&head, 0);    /* RSV = 0 */ //添加数据到参数一
    buf_write_u8(&head, 0);     /* FRAG = 0 */ //添加数据到参数一
    buf_write_u8(&head, '\x01'); /* ATYP = 1 (IP V4) */ //添加数据到参数一
    buf_write(&head, &to->dest.addr.in4.sin_addr, sizeof(to->dest.addr.in4.sin_addr));//将参数三长度的参数二拷贝到参数一
