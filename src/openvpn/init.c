...
void
context_clear(struct context *c)//置空
{
    CLEAR(*c);
}

void
context_clear_1(struct context *c)//置空level_1上下文结构体
{
    CLEAR(c->c1);
}

void
context_clear_2(struct context *c)//置空level_2上下文结构体
{
    CLEAR(c->c2);
}
...
run_up_down(const char *command,
            ...
            struct env_set *es)//将隧道端点和MTU解析器传递给用户提供的脚本 用于执行向上/向下的脚本
{
    struct gc_arena gc = gc_new();

    if (signal_text)
    {
        setenv_str(es, "signal", signal_text);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
    }
    setenv_str(es, "script_context", context);
    setenv_int(es, "tun_mtu", tun_mtu);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
    setenv_int(es, "link_mtu", link_mtu);
    setenv_str(es, "dev", arg);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
    if (dev_type)
    {
        setenv_str(es, "dev_type", dev_type);
    }
#ifdef _WIN32
    setenv_int(es, "dev_idx", adapter_index);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
#endif
    ...
    if (plugin_defined(plugins, plugin_type))//return 0
    {
    ...
    if (command)
    {
        struct argv argv = argv_new();//返回一个初始化过的结构体
        ASSERT(arg);
        setenv_str(es, "script_type", script_type);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
        argv_parse_cmd(&argv, command);//解析配置文件 添加到参数一中
        argv_printf_cat(&argv, "%s %d %d %s %s %s", arg, tun_mtu, link_mtu,
                        ifconfig_local, ifconfig_remote, context);
        argv_msg(M_INFO, &argv);//打印信息
 ...
 */
static void
init_connection_list(struct context *c)//复位连接链表 还可能随机打乱链表顺序
{
 ...
 */
static void
next_connection_entry(struct context *c)//递增到下一个连接条目
{
    struct connection_list *l = c->options.connection_list;
    bool ce_defined;
 ...
 */
void
init_query_passwords(const struct context *c)//查询私钥和auth- user- pass用户名/密码
{
#ifdef ENABLE_CRYPTO
...
static void
init_proxy_dowork(struct context *c)//初始化代理对象信息
{
    bool did_http = false;

    uninit_proxy_dowork(c);//释放参数空间
    ...
    if (!did_http && c->options.ce.socks_proxy_server)
    {
        c->c1.socks_proxy = socks_proxy_new(c->options.ce.socks_proxy_server,
                                            ...
                                            c->options.ce.socks_proxy_authfile);//分配空间 使用参数创建新的代理
        if (c->c1.socks_proxy)
        {
...
void
context_init_1(struct context *c)//置空level_1上下文结构体 复位连接链表 还可能随机打乱链表顺序
{
    context_clear_1(c);//置空level_1上下文结构体

    packet_id_persist_init(&c->c1.pid_persist);//空函数体

    init_connection_list(c);//复位连接链表 还可能随机打乱链表顺序
...
void
context_gc_free(struct context *c)//释放垃圾回收器
{
    gc_free(&c->c2.gc);
...
init_static(void)//静态 全局等变量初始化
{
     ...
     */
    struct timeval tv;
    if (!gettimeofday(&tv, NULL))	//注意windows和linux下兼容，返回时间
    {
        const unsigned int seed = (unsigned int) tv.tv_sec ^ tv.tv_usec;
        srandom(seed);	//srandom与random配合使用，srand与rand配合使用
    }

    error_reset();              /* initialize error.c */ //全局IO信息等设置初始化
...
void
init_verb_mute(struct context *c, unsigned int flags)//设置冗度和静音级别
{
    if (flags & IVM_LEVEL_1)
    {
        /* set verbosity and mute levels */
        set_check_status(D_LINK_ERRORS, D_READ_WRITE);//设置检查状态
        set_debug_level(c->options.verbosity, SDL_CONSTRAIN);//设置调试等级
...
/*
 * Persistent TUN/TAP device management mode?持续的TUN/TAP设备管理模式
 */
bool
do_persist_tuntap(const struct options *options)
{
    if (options->persist_config)
    {
        ...
        notnull(options->dev, "TUN/TAP device (--dev)");//检查第一个参数 为空则输出提示信息
            ...
            )
        {
            msg(M_FATAL|M_OPTERR,
                "options --mktun or --rmtun should only be used together with --dev");//需要一起配置的提示信息
        }
 ...
 */
bool
possibly_become_daemon(const struct options *options)//变为守护进程的相关设置
{
    bool ret = false;
    ...
    if (options->daemon)
    {
        ...
        if (daemon(1, options->log) < 0)//库函数 变为守护进程运行
        {
            msg(M_ERR, "daemon() failed or unsupported");
        }
        restore_signal_state();//在daemonization之后调用保留信号设置
        if (options->log)
        {
            set_std_files_to_null(true);//设置标准文件描述符为/dev/null
        }
 ...
 */
static void
do_uid_gid_chroot(struct context *c, bool no_delay)//如果需要的话，可以使用UID / GID降级，chroot和SELinux上下文切换
{
...
void
reset_coarse_timers(struct context *c)//重置为0
{
 ...
 */
static void
do_init_server_poll_timeout(struct context *c)//初始化连接超时计时器
{
    update_time();
 ...
 */
static void
do_init_timers(struct context *c, bool deferred)//初始化定时器
{
    update_time();
 ...
 */
static void
do_init_traffic_shaper(struct context *c)//初始化输出速度限幅器
{
 ...
 */
static void
do_alloc_route_list(struct context *c)//分配route_list
{
    if (!c->c1.route_list)
    {
        ALLOC_OBJ_CLEAR_GC(c->c1.route_list, struct route_list, &c->gc);//分配空间 创建并关联垃圾回收条目 并返回数据区偏移地址
    }
...
do_init_route_list(const struct options *options,
                   ...
                   struct env_set *es)//初始化路由链表 保存路由信息到环境变量
{
    const char *gw = NULL;
    int dev = dev_type_enum(options->dev, options->dev_type);//根据设备名称确定通道类型
    int metric = 0;
                        ...
                        es))//初始化路由链表信息
    {
        ...
        setenv_routes(es, route_list);//拷贝路由信息到环境变量
    }
}
...
/*
 * Called after all initialization has been completed. 在所有初始化完成后调用。
 */
void
initialization_sequence_completed(struct context *c, const unsigned int flags)//监听/连接套接字
{
    static const char message[] = "Initialization Sequence Completed";

    /* Reset the unsuccessful connection counter on complete initialisation 重置失败的连接计数器完成初始化*/
    c->options.unsuccessful_attempts = 0;

    /* If we delayed UID/GID downgrade or chroot, do it now 如果我们推迟了UID/GID降级或chroot，现在就开始吧*/
     ...
     * 在某些情况下(例如通过推覆来接收auth- token)，在客户机上配置的auth- nocache选项被重写;出于这个原因，
     * 我们必须在尝试擦除用户输入的用户/ pass之前等待下推消息
     */
    ...
    /* Flag that we initialized 我们初始化的标记*/
...
#ifdef ENABLE_MANAGEMENT
    /* Tell management interface that we initialized 告诉管理界面我们初始化了*/
    if (management)
    {
        ...
        if (tun_local)
        {
            management_post_tunnel_open(management, *tun_local);//监听/连接套接字
        }
    }
...
do_route(const struct options *options,
         ...
         const struct plugin_list *plugins,
         struct env_set *es)//可能会根据选项添加路由和/或调用routeup脚本
{
    ...
    if (options->route_script)
    {
        ...
        argv_parse_cmd(&argv, options->route_script);//解析配置文件 添加到参数一中
 ...
 */
static void
do_init_tun(struct context *c)//初始化tun/tap对象
{
    c->c1.tuntap = init_tun(c->options.dev,
                            ...
                            c->c2.es);//初始化tun/tap对象

    init_tun_post(c->c1.tuntap,
                  &c->c2.frame,
                  &c->options.tuntap_options);//参数一初始化
...
static bool
do_open_tun(struct context *c)//打开的tun/tap设备，ifconfig，调用脚本，等等
{
    ...
    do_alloc_route_list(c);//分配route_list
    ...
    if (c->options.routes && c->c1.route_list)
    {
        do_init_route_list(&c->options, c->c1.route_list,
                           &c->c2.link_socket->info, c->c2.es);//初始化路由链表 保存路由信息到环境变量
    }
    if (c->options.routes_ipv6 && c->c1.route_ipv6_list)
    {
        ...
        && ifconfig_order() == IFCONFIG_BEFORE_TUN_OPEN)
    {
        ...
        const char *guess = guess_tuntap_dev(c->options.dev,
                                             c->options.dev_type,
                                             c->options.dev_node,
                                             &gc);//在设备实际打开之前，尝试预测实际的TUN/TAP设备实例名
        do_ifconfig(c->c1.tuntap, guess, TUN_MTU_SIZE(&c->c2.frame), c->c2.es);//通过shell执行ifconfig命令
    }
    ...
    if (route_order() == ROUTE_BEFORE_TUN)
    {
        ...
        do_route(&c->options, c->c1.route_list, c->c1.route_ipv6_list,
                 c->c1.tuntap, c->plugins, c->c2.es);//可能会根据选项添加路由和/或调用routeup脚本
    }
#ifdef TARGET_ANDROID
    ...
    if (c->options.lladdr)
    {
        set_lladdr(c->c1.tuntap->actual_name, c->options.lladdr, c->c2.es);//设置硬件地址
    }
    ...
    {
        do_ifconfig(c->c1.tuntap, c->c1.tuntap->actual_name, TUN_MTU_SIZE(&c->c2.frame), c->c2.es);//通过shell执行ifconfig命令
    }
    ...
    run_up_down(c->options.up_script,
                ...
                NULL,
                "up",
                c->c2.es);//将隧道端点和MTU解析器传递给用户提供的脚本 用于执行向上/向下的脚本
    ...
    if (c->c1.tuntap->post_open_mtu)
    {
        frame_set_mtu_dynamic(&c->c2.frame,
                              ...
                              SET_MTU_TUN | SET_MTU_UPPER_BOUND);//动态设置tun MTU
    }
...
else
{
    ...
    do_ifconfig_setenv(c->c1.tuntap, c->c2.es);//设置ifconfig_*环境变量
...
bool
do_up(struct context *c, bool pulled_options, unsigned int option_types_found)//处理拉下的选项 执行脚本？
{
    if (!c->c2.do_up_ran)
    {
        reset_coarse_timers(c);//重置为0

        if (pulled_options)
        {
            if (!do_deferred_options(c, option_types_found))//处理non-tun-related拉下的选项
            {
        ...
        {
            c->c2.did_open_tun = do_open_tun(c);//打开的tun/tap设备，ifconfig，调用脚本，等等
            update_time();
        ...
        if (c->c2.did_open_tun)
        {
            ...
            /* if --route-delay was specified, start timer 开始定时器*/
            if ((route_order() == ROUTE_AFTER_TUN) && c->options.route_delay_defined)
            {
 ...
 */
bool
do_deferred_options(struct context *c, const unsigned int found)//处理non-tun-related拉下的选项
{
    if (found & OPT_P_MESSAGES)
    {
        init_verb_mute(c, IVM_LEVEL_1|IVM_LEVEL_2);//设置冗度和静音级别
        msg(D_PUSH, "OPTIONS IMPORT: --verb and/or --mute level changed");
    }
    if (found & OPT_P_TIMER)
    {
        do_init_timers(c, true);//初始化定时器
    ...
    if (found & OPT_P_SHAPER)
    {
        ...
        do_init_traffic_shaper(c);//初始化输出速度限幅器
    }
 ...
 */
static void
socket_restart_pause(struct context *c)//重启前睡眠相关设置
{
    int sec = 2;
    int backoff = 0;
    ...
    if (sec)
    {
        ...
        management_sleep(sec);  //sleep(sec)
    }
}
 ...
 */
static void
do_startup_pause(struct context *c)//对context_2初始化进行可能的暂停
{
    if (!c->first_time)
    {
        socket_restart_pause(c);//重启前睡眠相关设置
    }
    else
    {
...
static void
do_init_crypto(struct context *c, const unsigned int flags)//初始化加密层？
{
#ifdef ENABLE_CRYPTO
...
static void
do_option_warnings(struct context *c)//打印警告或错误信息
{
...
init_context_buffers(const struct frame *frame)//根据参数分配多个堆空间并置空 返回地址
{
    struct context_buffers *b;

    ALLOC_OBJ_CLEAR(b, struct context_buffers);//分配空间并置空

    b->read_link_buf = alloc_buf(BUF_SIZE(frame));//分配堆空间并置空
    b->read_tun_buf = alloc_buf(BUF_SIZE(frame));//分配堆空间并置空

    b->aux_buf = alloc_buf(BUF_SIZE(frame));//分配堆空间并置空
...
{
    c->c2.buffers = init_context_buffers(&c->c2.frame);//根据参数分配多个堆空间并置空 返回地址
 ...
 */
static void
do_link_socket_new(struct context *c)//分配空间并初始化
{
    ASSERT(!c->c2.link_socket);
    c->c2.link_socket = link_socket_new();//分配空间并初始化
 ...
 */
static void
do_init_socket_1(struct context *c, const int mode)//socket初始化
{
 ...
 */
static void
do_init_socket_2(struct context *c)//完成TCP/UDP套接字
{
 ...
 */
static void
do_print_data_channel_mtu_parms(struct context *c)//格式化输出参数->c2.frame信息
{
    frame_print(&c->c2.frame, D_MTU_INFO, "Data Channel MTU parms");//格式化输出参数一信息
#ifdef ENABLE_FRAGMENT
    if (c->c2.fragment)
    {
 ...
 */
static void
do_compute_occ_strings(struct context *c)//获取本地和远程选项的兼容性字符串
{
    struct gc_arena gc = gc_new();
 ...
 */
static void
do_init_first_time(struct context *c)//在每个程序实例化时执行一次 有可能变成守护进程
{
    if (c->first_time && !c->c0)
    {
        ...
        platform_nice(c->options.nice);//改变程序优先级
    }
}
...
do_event_set_init(struct context *c,
                  bool need_us_timeout)//设定事件调用函数(epoll select poll win下。。)
{
    ...
    c->c2.event_set = event_set_init(&c->c2.event_set_max, flags);//设定事件调用函数(epoll select poll win下。。)
...
static void
do_open_status_output(struct context *c)//打开文件 生成输出状态信息
{
    if (!c->c1.status_output)
    {
        c->c1.status_output = status_open(c->options.status_file,
                                          ...
                                          -1,
                                          NULL,
                                          STATUS_OUTPUT_WRITE);//打开文件 生成输出状态信息
...
static void
do_inherit_env(struct context *c, const struct env_set *src)//继承参数二环境变量
{
    c->c2.es = env_set_create(NULL);//开辟环境变量空间 并初始化后返回
    c->c2.es_owned = true;
    env_set_inherit(c->c2.es, src);//将参数二所含节点副本逐个移动到参数一所含链表的头部
}
 ...
 */
static void
do_setup_fast_io(struct context *c)//是否可以开启Fast I/O
{
...
init_management(struct context *c)
{
    if (!management)
    {
        management = management_init();	//管理对象初始化
    }
}

bool
open_management(struct context *c)//开放管理子系统 含 监听/连接套接字
{
    /* initialize management layer 初始化管理层*/
    if (management)
    {
 ...
 * signal settings.
 */ //初始化一个隧道实例，处理pre - init信号设置
void
init_instance_handle_signals(struct context *c, const struct env_set *env, const unsigned int flags)
{
    pre_init_signal_catch();//注册信号处理函数
    init_instance(c, env, flags);//初始化一个隧道实例
    post_init_signal_catch();//注册信号处理函数
     ...
     * a management hold.
     */
    if (IS_SIG(c))//这样做是为了让在初始化过程中抛出的信号可以让我们回到管理控制
    {
        remap_signal(c);//处理信号
 ...
 */
void
init_instance(struct context *c, const struct env_set *env, const unsigned int flags)//初始化一个隧道实例
{
    const struct options *options = &c->options;
    ...
    if (env)
    {
        do_inherit_env(c, env);//继承参数二环境变量
    }
    ...
    {
        do_startup_pause(c);//对context_2初始化进行可能的暂停
        if (IS_SIG(c))//如果接收了一个信号
        {
            goto sig;
        }
    }
    ...
    {
        do_preresolve(c);//确保主机名、服务器名存入现有参数的c1.dns_cache成员中 负责设定软中断信号
        if (IS_SIG(c))
        {
            goto sig;
        }
    ...
    next_connection_entry(c);//递增到下一个连接条目
    ...
    if (c->mode == CM_P2P || c->mode == CM_TOP)
    {
        do_option_warnings(c);//打印警告或错误信息
    }
    ...
    if (c->mode == CM_P2P || c->mode == CM_TOP)
    {
        do_setup_fast_io(c);//是否可以开启Fast I/O
    }

    /* should we throw a signal on TLS errors? */
    do_signal_on_tls_errors(c);//是否抛出信号中断

    /* open --status file */
    if (c->mode == CM_P2P || c->mode == CM_TOP)
    {
        do_open_status_output(c);//打开文件 生成输出状态信息
    }
    ...
    {
        c->c2.occ_op = occ_reset_op();//重置值为-1
    }
#endif
    ...
    if (c->mode == CM_P2P)
    {
        do_event_set_init(c, SHAPER_DEFINED(&c->options));//设定事件调用函数(epoll select poll win下。。)
    }
    else if (c->mode == CM_CHILD_TCP)
    {
        do_event_set_init(c, false);//设定事件调用函数(epoll select poll win下。。)
    }

    /* initialize HTTP or SOCKS proxy object at scope level 2 */
    init_proxy(c);//初始化代理对象信息

    /* allocate our socket object */
    if (c->mode == CM_P2P || c->mode == CM_TOP || c->mode == CM_CHILD_TCP)
    {
        do_link_socket_new(c);//分配空间并初始化
    }
    ...
    /* init crypto layer */ //初始化加密层
    {
        ...
        do_init_crypto(c, crypto_flags);//初始化加密层？
        if (IS_SIG(c) && !child)
        {
            goto sig;
        }
    ...
    {
        do_init_buffers(c);//根据参数分配多个堆空间并置空 返回地址
    }
    ...
    if (c->mode == CM_P2P || c->mode == CM_TOP || c->mode == CM_CHILD_TCP)
    {
        do_init_socket_1(c, link_socket_mode);//socket初始化
    }
    ...
    /* print MTU info */
    do_print_data_channel_mtu_parms(c);//格式化输出参数->c2.frame信息

#ifdef ENABLE_OCC
    /* get local and remote options compatibility strings */
    if (c->mode == CM_P2P || child)
    {
        do_compute_occ_strings(c);//获取本地和远程选项的兼容性字符串
    }
#endif
    ...
    do_init_first_time(c);//在每个程序实例化时执行一次 有可能变成守护进程
    ...
    do_init_server_poll_timeout(c);//初始化连接超时计时器

    /* finalize the TCP/UDP socket */
    if (c->mode == CM_P2P || c->mode == CM_TOP || c->mode == CM_CHILD_TCP)
    {
        do_init_socket_2(c);//完成TCP/UDP套接字
    }
     ...
     */
    do_uid_gid_chroot(c, c->c2.did_open_tun);//如果需要的话，可以使用UID / GID降级，chroot和SELinux上下文切换

    /* initialize timers */
    if (c->mode == CM_P2P || child)
    {
        do_init_timers(c, false);//初始化定时器
    }
...
sig:
    ...
    close_context(c, -1, flags);//结束上下文动作
    return;
}
 ...
 */
void
close_instance(struct context *c)//关闭一个隧道实例
{
    /* close event objects */
                      ...
                      const struct context *src)
{
    ...
    /* context init */
    init_instance(dest, src->c2.es, CC_NO_CLOSE | CC_USR1_TO_HUP);//初始化一个隧道实例
    if (IS_SIG(dest))
    {
        return;
    }
...
void
close_context(struct context *c, int sig, unsigned int flags)//结束上下文动作
{
    ASSERT(c);
    ASSERT(c->sig);
