...
int
parse_signal(const char *signame)//解析信号名字 返回对应的信号值
{
    int i;
...
const char *
signal_name(const int sig, const bool upper)//返回信号的大写或小写名字
{
    int i;
    for (i = 0; i < (int)SIZE(signames); ++i)
    {
...
void
throw_signal(const int signum)//抛出信号的全局设置
{
...
void
throw_signal_soft(const int signum, const char *signal_text)//信号软中断
{
    siginfo_static.signal_received = signum;
    siginfo_static.source = SIG_SOURCE_SOFT;//软中断？
    siginfo_static.signal_text = signal_text;//相关信息说明
}

static void
signal_reset(struct signal_info *si)//重置各成员值
{
    if (si)
    {
...
void
print_signal(const struct signal_info *si, const char *title, int msglevel)//打印接收到的信号信息
{
    if (si)
    {
        ...
        switch (si->signal_received)
        {
            ...
            case SIGUSR1:
                msg(msglevel, "%s[%s,%s] received, %s restarting",
                    signal_name(si->signal_received, true), hs, type, t);//打印信号的大写或小写名字
                break;
...
{
    throw_signal(signum);
    signal(signum, signal_handler);		//适配旧版本的signal():调用一次后重置信号处理函数SIG_DFL
}
...
void
pre_init_signal_catch(void)//注册信号处理函数
{
#ifndef _WIN32
#ifdef HAVE_SIGNAL_H
...
void
post_init_signal_catch(void)//注册信号处理函数
{
#ifndef _WIN32
#ifdef HAVE_SIGNAL_H
...
void
restore_signal_state(void)//在daemonization之后调用保留信号设置
{
#ifdef HAVE_SIGNAL_H
 ...
 */
void
print_status(const struct context *c, struct status_output *so)//打印统计信息
{
    struct gc_arena gc = gc_new();//返回一个已初始化好的gc_arena临时变量 用于给外部变量赋值 没有分配内存

    status_reset(so);//文件偏移重置

    status_printf(so, "OpenVPN STATISTICS");//格式化写文件 并调用虚拟接口
    ...
    if (tuntap_defined(c->c1.tuntap))//若定义了参数 返回1
    {
        status_printf(so, "TAP-WIN32 driver status,\"%s\"",
                      tap_win_getinfo(c->c1.tuntap, &gc));//格式化写文件 并调用虚拟接口
    }
#endif

    status_printf(so, "END");//格式化写文件 并调用虚拟接口
    status_flush(so);//初始化参数
    gc_free(&gc);
}
...
static void
process_explicit_exit_notification_init(struct context *c)//处理明确退出通知的触发和时间等待
{
    msg(M_INFO, "SIGTERM received, sending exit notification to peer");
    event_timeout_init(&c->c2.explicit_exit_notification_interval, 1, 0);//使用参数二三初始化参数一
    reset_coarse_timers(c);//重置为0
    signal_reset(c->sig);//重置各成员值
    halt_non_edge_triggered_signals();
    c->c2.explicit_exit_notification_time_wait = now;
}

void
process_explicit_exit_notification_timer_wakeup(struct context *c)//发送OCC_EXIT信息给远端
{
...
void
remap_signal(struct context *c)//处理信号值
{
...
static void
process_sigusr2(const struct context *c)//自定义信号处理
{
    struct status_output *so = status_open(NULL, 0, M_INFO, NULL, 0);//打开文件 生成输出状态信息
    print_status(c, so);//打印统计信息
    status_close(so);//关闭文件 释放参数 出错返回0
    signal_reset(c->sig);//重置各成员值
}

static bool
process_sigterm(struct context *c)//中断信号处理
{
    bool ret = true;
#ifdef ENABLE_OCC
    ...
    {
        process_explicit_exit_notification_init(c);//处理明确退出通知的触发和时间等待
        ret = false;
    }
#endif
    return ret;
 ...
 */
static bool
ignore_restart_signals(struct context *c)//忽略信号与不忽略信号处理
{
    bool ret = false;
#ifdef ENABLE_OCC
         ...
         && event_timeout_defined(&c->c2.explicit_exit_notification_interval) )
    {
        ...
        else
        {
            msg(M_INFO, "Converting soft %s received during exit notification to SIGTERM",
                signal_name(c->sig->signal_received, true));
            register_signal(c, SIGTERM, "exit-with-notification");//注册接收的信号
            ret = false;
        }
    }
#endif
...
bool
process_signal(struct context *c)//信号各种处理
{
    bool ret = true;

    if (ignore_restart_signals(c))//忽略信号与不忽略信号处理
    {
        ret = false;
    }
    else if (c->sig->signal_received == SIGTERM || c->sig->signal_received == SIGINT)
    {
        ret = process_sigterm(c);//中断信号处理
    }
    else if (c->sig->signal_received == SIGUSR2)
    {
        process_sigusr2(c);//自定义信号处理
        ret = false;
    }
    return ret;
}
...
void
register_signal(struct context *c, int sig, const char *text)//注册接收的信号
{
