...
struct event_set_return
{
    unsigned int rwflags;//事件类型
    void *arg;//触发事件绑定的参数指针arg
};
...
static inline void
event_ctl(struct event_set *es, event_t event, unsigned int rwflags, void *arg)//调用事件修改模式 调整事件可读可写监测状态
{
    (*es->func.ctl)(es, event, rwflags, arg);
}

static inline int
event_wait(struct event_set *es, const struct timeval *tv, struct event_set_return *out, int outlen)//等待事件发生 如epoll_wait()
{
    int ret;
    perf_push(PERF_IO_WAIT);//空函数体
    ret = (*es->func.wait)(es, tv, out, outlen);//等待事件发生 如epoll_wait() 触发事件拷贝给参数三
    perf_pop();//空函数体
    return ret;
}
