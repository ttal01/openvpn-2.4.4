...
static inline void
mroute_get_in_addr_t(struct mroute_addr *ma, const in_addr_t src, unsigned int mask)//参数一中存入ip源地址
{
    if (ma)
    {
...
mroute_extract_addr_arp(struct mroute_addr *src,
                        ...
                        const struct buffer *buf)//arp提取参数三ip地址存入参数一参数二中
{
    unsigned int ret = 0;
    if (BLEN(buf) >= (int) sizeof(struct openvpn_arp))
    {
        ...
        {
            mroute_get_in_addr_t(src, arp->ip_src, MR_ARP);//参数一中存入ip源地址
            mroute_get_in_addr_t(dest, arp->ip_dest, MR_ARP);//参数一中存入ip目的地址
...
mroute_extract_addr_ip(struct mroute_addr *src, struct mroute_addr *dest,
                       const struct buffer *buf)//取出参数三中的ip地址分别存入参数一和参数二中
{
    unsigned int ret = 0;
    if (BLEN(buf) >= 1)//返回当前缓冲区的内部数据长度
    {
        switch (OPENVPN_IPH_GET_VER(*BPTR(buf)))//buf第一个字符右移4位&0x0F
        {
            case 4:
                if (BLEN(buf) >= (int) sizeof(struct openvpn_iphdr))
                {
                    ...
                    mroute_get_in_addr_t(src, ip->saddr, 0);//参数一中存入ip4源地址
                    mroute_get_in_addr_t(dest, ip->daddr, 0);//参数一中存入ip4目的地址
            ...
            case 6:
                if (BLEN(buf) >= (int) sizeof(struct openvpn_ipv6hdr))
                {
                    ...
                    mroute_get_in6_addr(src, ipv6->saddr, 0);//参数一中存入ip6源地址
                    mroute_get_in6_addr(dest, ipv6->daddr, 0);//参数一中存入ip6目的地址
...
mroute_extract_addr_ether(struct mroute_addr *src,
                          ...
                          const struct buffer *buf) //提取参数5mac地址存入参数一参数二（ip存入3.4）中
{
    unsigned int ret = 0;
    if (BLEN(buf) >= (int) sizeof(struct openvpn_ethhdr))
    {
        ...
        if (src)
        {
            ...
            src->netbits = 0;
            src->len = 6;
            memcpy(src->eth_addr, eth->source, sizeof(dest->eth_addr));//拷贝源mac地址
        }
        if (dest)
        {
            ...
            dest->netbits = 0;
            dest->len = 6;
            memcpy(dest->eth_addr, eth->dest, sizeof(dest->eth_addr));//拷贝目标mac地址
        ...
        {
            struct buffer b = *buf;
            if (buf_advance(&b, sizeof(struct openvpn_ethhdr)))//数据消除回退 用于按size大小记录读取的数据位置
            {
                switch (ntohs(eth->proto))
                {
                    case OPENVPN_ETH_P_IPV4://取出参数三中的ip地址分别存入参数一和参数二中
                        ret |= (mroute_extract_addr_ip(esrc, edest, &b) << MROUTE_SEC_SHIFT);
                        break;

                    case OPENVPN_ETH_P_ARP://arp提取参数三ip地址存入参数一参数二中
...
mroute_helper_init(int ageable_ttl_secs)
{
    ...
    ALLOC_OBJ_CLEAR(mh, struct mroute_helper);//分配空间并置空
