 ...
 * 返回最早唤醒时间的节点。如果两个节点有完全相同的唤醒时间，则根据分配给每个节点的随机优先级(在每次重新添加一个条目时将优先级随机分配)。
 */
static inline struct schedule_entry *
schedule_get_earliest_wakeup(struct schedule *s,
                             struct timeval *wakeup)//返回最早唤醒时间的节点
{
    ...
    {
        s->earliest_wakeup = schedule_find_least(s->root);//返回最早的事件
    }
