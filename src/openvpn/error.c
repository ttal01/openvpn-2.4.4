...
bool
set_debug_level(const int level, const unsigned int flags)//设置调试等级
{
...
void
error_reset(void)//全局IO信息等设置初始化
{
    use_syslog = std_redir = false;//是否写入syslog
    suppress_timestamps = false;//是否包含时间戳
    machine_readable_output = false;//是否可解析 并且有前缀
    x_debug_level = 1;
    mute_cutoff = 0;
    mute_count = 0;
    mute_category = 0;
    default_out = OPENVPN_MSG_FP;//默认标准输出
    default_err = OPENVPN_MSG_FP;//默认标准错误
...
void
set_check_status(unsigned int info_level, unsigned int verbose_level)//设置检查状态
{
 ...
 * from the OS.
 * 通过内联函数check_status()调用大多数套接字或tun / tap操作。
 * 决定是否应该打印错误消息，并查看是否可以从错误中提取任何有用的信息，例如从操作系统中提示的路径MTU。
 */
void
x_check_status(int status,
               const char *description,
               struct link_socket *sock,
               struct tuntap *tt)//决定是否应该打印错误消息，并查看是否可以从错误中提取任何有用的信息
{
...
#elif defined(_WIN32)
        /* get possible driver error from TAP-Windows driver 从tap-windows驱动程序中获取可能的驱动程序错误*/
...
const char *
strerror_win32(DWORD errnum, struct gc_arena *gc)//返回Windows错误信息
{
    /*
    ...
    /* format a windows error message */ //格式化windows错误信息
    {
        char message[256];
        struct buffer out = alloc_buf_gc(256, gc);//返回局部缓冲区变量 内含已注册关联的堆空间信息
