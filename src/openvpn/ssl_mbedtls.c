...
tls_ctx_server_new(struct tls_root_ctx *ctx)
{
    ...
    ALLOC_OBJ_CLEAR(ctx->dhm_ctx, mbedtls_dhm_context);//分配空间并置空

    ALLOC_OBJ_CLEAR(ctx->ca_chain, mbedtls_x509_crt);//分配空间并置空
                       ...
                       )
{
    ...
    if (!ctx->crt_chain)
    {
        ALLOC_OBJ_CLEAR(ctx->crt_chain, mbedtls_x509_crt);//分配空间并置空
    }
                       ...
                       )
{
    ...
    if (!ctx->priv_key)
    {
        ALLOC_OBJ_CLEAR(ctx->priv_key, mbedtls_pk_context);//分配空间并置空
    }
                                 ...
                                 const char *cert_file, const char *cert_file_inline)
{
    ...
    ALLOC_OBJ_CLEAR(ctx->external_key, struct external_context);//分配空间并置空
                           ...
                           const char *crl_inline)
{
    ...
    if (ctx->crl == NULL)
    {
        ALLOC_OBJ_CLEAR(ctx->crl, mbedtls_x509_crl);//分配空间并置空
    }
    mbedtls_x509_crl_free(ctx->crl);
                   ...
                   const struct tls_root_ctx *ssl_ctx, bool is_server, struct tls_session *session)
{
    ...
    ALLOC_OBJ_CLEAR(ks_ssl->ctx, mbedtls_ssl_context);//分配空间并置空
