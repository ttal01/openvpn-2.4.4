 ...
 * addresses of the packet.在buf中给出一个原始数据包，返回src和dest地址。
 */
                                ...
                                int tunnel_type)
{
    ...
    {
        ret = mroute_extract_addr_ip(src, dest, buf);//取出参数三中的ip地址分别存入参数一和参数二中
    }
    else if (tunnel_type == DEV_TYPE_TAP)
    {
        ret = mroute_extract_addr_ether(src, dest, esrc, edest, buf);//提取参数5mac地址存入参数一参数二（ip存入3.4）中
    }
    return ret;
}

static inline bool
mroute_addr_equal(const struct mroute_addr *a1, const struct mroute_addr *a2)//比较两个参数 相同返回1
{
    if (a1->type != a2->type)
    {
        return false;
