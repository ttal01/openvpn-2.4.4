 ...
 */
#include "syshead.h"    //增加 --liut 171116
#if P2MP_SERVER
...
static inline void *
hash_lookup(struct hash *hash, const void *key)//返回已存在的并移动为头节点的->value成员
{
    void *ret = NULL;
    ...
    he = hash_lookup_fast(hash, bucket, key, hv);//找到节点 移动到链表头部 返回该节点
    if (he)
    {
        ret = he->value;
    }
