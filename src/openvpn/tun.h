...
static inline bool
tuntap_defined(const struct tuntap *tt)//若定义了参数 返回1
{
#ifdef _WIN32
...
tun_set(struct tuntap *tt,
        ...
        void *arg,
        unsigned int *persistent)//tun tap事件控制设置
{
    if (tuntap_defined(tt))//若定义了参数 返回1
    {
        /* if persistent is defined, call event_ctl only if rwflags has changed since last call */
        //如果已经定义了持久性，那么仅当自上次调用时rwflags更改后 才调用event_ctl
        if (!persistent || *persistent != rwflags)
        {
            event_ctl(es, tun_event_handle(tt), rwflags, arg);//调用事件修改模式 调整事件可读可写监测状态
            if (persistent)
            {
                *persistent = rwflags;
            }
