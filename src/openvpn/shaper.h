...
/*#define SHAPER_DEBUG*/
#include "syshead.h"    //增加 --liut 171115
 ...
 * time, or 0 if no delay.在相对于当前时间的微秒内返回流量延迟，如果不延迟，则返回0
 */
static inline int
shaper_delay(struct shaper *s)
{
    ...
    if (tv_defined(&s->wakeup))//参数的秒及微妙均>0 返回1
    {
        ASSERT(!openvpn_gettimeofday(&tv, NULL));//获取当前时间
        delay = tv_subtract(&s->wakeup, &tv, SHAPER_MAX_TIMEOUT);//返回参数二 - 参数一
#ifdef SHAPER_DEBUG
...
/*
 * We are about to send a datagram of nbytes bytes.我们将要发送一个nbytes字节的数据报。
 *
 * Compute when we can send another datagram,
 * based on target throughput (s->bytes_per_second).根据目标吞吐量 计算何时发送另一个数据报。
 */
static inline void
shaper_wrote_bytes(struct shaper *s, int nbytes)//发送一个nbytes字节的数据报 根据目标吞吐量 计算何时发送另一个数据报
{
    struct timeval tv;
