 ...
 * date/time in a struct timeval. 返回一个ascii字符串，描述在结构时间内的绝对日期/时间
 */
const char *
tv_string_abs(const struct timeval *tv, struct gc_arena *gc)
{
    return time_string((time_t) tv->tv_sec,
                       (int) tv->tv_usec,
                       true,
                       gc);//将time_t格式化为ascii，或在0时使用当前时间
}

/* format a time_t as ascii, or use current time if 0 将time_t格式化为ascii，或在0时使用当前时间*/
