 ...
 */
void
set_std_files_to_null(bool stdin_only)//设置标准文件描述符为/dev/null
{
#if defined(HAVE_DUP) && defined(HAVE_DUP2)
    int fd;
    if ((fd = open("/dev/null", O_RDWR, 0)) != -1)
    {
        dup2(fd, 0);//拷贝文件描述符 fd->0
        if (!stdin_only)
        {
            dup2(fd, 1);
            dup2(fd, 2);
 ...
 * 在fork()中运行execve()。设计用于复制system()的语义，但更安全的方式是不需要调用shell或使用格式化和解析命令行来处理的风险。
 */
int
...
static char *
construct_name_value(const char *name, const char *value, struct gc_arena *gc)//根据参数大小开辟空间 添加“参数一=参数二” 返回当前缓冲区的内部数据偏移地址
{
    struct buffer out;
    ...
    out = alloc_buf_gc(strlen(name) + strlen(value) + 2, gc);//返回局部缓冲区变量 内含已注册关联的堆空间信息
    buf_printf(&out, "%s=%s", name, value);//格式化输出（附加）到缓冲区中
    return BSTR(&out);//返回当前缓冲区的内部上一条数据结尾偏移地址（即当前数据开始地址）
}

static bool
env_string_equal(const char *s1, const char *s2)//判断两个字符串”=“前部分是否相等 相等返回1
{
    int c1, c2;
    ASSERT(s1);
    ASSERT(s2);
...
static bool
remove_env_item(const char *str, const bool do_free, struct env_item **list)//找到链表中参数一所在节点并删除
{
    ...
    for (current = *list, prev = NULL; current != NULL; current = current->next)//迭代链表 找到与str相等的条目
    {
        if (env_string_equal(current->string, str))//判断两个字符串”=“前部分是否相等 相等返回1
        {
            if (prev)
            {
                prev->next = current->next;//移除节点
            }
            else
            {
                *list = current->next;//移除节点
            }
            if (do_free)
            {
        ...
        prev = current;//指向前一个节点的指针后移
    }
    return false;
}

static void
add_env_item(char *str, const bool do_alloc, struct env_item **list, struct gc_arena *gc)//将参数一（或副本）插入链表头位置
{
    ...
    ALLOC_OBJ_GC(item, struct env_item, gc);//分配空间 创建并关联垃圾回收条目 并返回数据区偏移地址到参数一
    item->string = do_alloc ? string_alloc(str, gc) : str;//指向str（或副本）地址
    item->next = *list;//新节点成员指向原链表的头
    *list = item;//更新链表头为新节点
}
...
static bool
env_set_del_nolock(struct env_set *es, const char *str)//找到链表中参数二所在节点并删除
{
    return remove_env_item(str, es->gc == NULL, &es->list);//找到链表中参数一所在节点并删除
}

static void
env_set_add_nolock(struct env_set *es, const char *str)//将参数二（副本）移动（没有则添加）到参数一所含链表的头部
{
    remove_env_item(str, es->gc == NULL, &es->list);//找到链表中参数一所在节点并删除
    add_env_item((char *)str, true, &es->list, es->gc);//将参数一（副本）插入链表头位置
}

struct env_set *
env_set_create(struct gc_arena *gc)//开辟环境变量空间 并初始化后返回
{
    struct env_set *es;
    ALLOC_OBJ_CLEAR_GC(es, struct env_set, gc);//分配空间 创建并关联垃圾回收条目 并返回数据区偏移地址到参数一
    es->list = NULL;
    es->gc = gc;
    return es;
}
...
void
env_set_destroy(struct env_set *es)//释放环境变量
{
    if (es && es->gc == NULL)
    {
...
env_set_del(struct env_set *es, const char *str)
{
    ...
    ASSERT(es);
    ASSERT(str);
    ret = env_set_del_nolock(es, str);//找到链表中参数二所在节点并删除
    return ret;
}
...
env_set_add(struct env_set *es, const char *str)
{
    ASSERT(es);
    ASSERT(str);
    env_set_add_nolock(es, str);//将参数二移动（没有则添加）到参数一所含链表的头部
}
...
void
env_set_inherit(struct env_set *es, const struct env_set *src)//将参数二所含节点副本逐个移动到参数一所含链表的头部
{
    const struct env_item *e;
    ...
    if (src)
    {
        ...
        while (e)
        {
            env_set_add_nolock(es, e->string);//将参数二（副本）移动（没有则添加）到参数一所含链表的头部
            e = e->next;
        }
    }
}
...
setenv_counter(struct env_set *es, const char *name, counter_type value)
{
    ...
    setenv_str(es, name, buf);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
}
...
{
    setenv_str_ex(es, name, value, CC_NAME, 0, 0, CC_PRINT, 0, 0);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
}
...
setenv_str_safe(struct env_set *es, const char *name, const char *value)
{
    ...
    {
        setenv_str(es, BSTR(&buf), value);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
    }
    else
    {
...
setenv_str_incr(struct env_set *es, const char *name, const char *value)
{
    ...
    if (counter < 1000)
    {
        setenv_str(es, tmpname, value);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
    }
    else
    {
...
{
    ASSERT(name);
    setenv_str(es, name, NULL);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
}
...
setenv_str_ex(struct env_set *es,
              ...
              const char value_replace)//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
{
    ...
    if (val_tmp)//成功替换后
    {
        //根据参数大小开辟空间 添加“参数一=参数二” 返回当前缓冲区的内部上一条数据结尾偏移地址（即参数一数据开始地址）
        const char *str = construct_name_value(name_tmp, val_tmp, &gc);
        env_set_add(es, str);//将参数二移动（没有则添加）到参数一所含链表的头部
    ...
    {
        env_set_del(es, name_tmp);//找到链表中参数二所在节点并删除
    }
 ...
 */
static const char *
setenv_format_indexed_name(const char *name, const int i, struct gc_arena *gc)//以name_i的格式添加 并返回起始地址
{
    struct buffer out = alloc_buf_gc(strlen(name) + 16, gc);//返回局部缓冲区变量 内含已注册关联的堆空间信息
    if (i >= 0)
    {
        buf_printf(&out, "%s_%d", name, i);//格式化输出（附加）到缓冲区中
    }
    else
    {
        buf_printf(&out, "%s", name);//格式化输出（附加）到缓冲区中
    }
    return BSTR(&out);//返回当前缓冲区的内部上一条数据结尾偏移地址（即本次添加数据的开始地址）
}

void
setenv_int_i(struct env_set *es, const char *name, const int value, const int i)//按"%s=%s"格式将后三个参数添加到参数一所含链表的头部
{
    struct gc_arena gc = gc_new();
    const char *name_str = setenv_format_indexed_name(name, i, &gc);//以name_i的格式添加 并返回起始地址
    setenv_int(es, name_str, value);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
    gc_free(&gc);
}

void
setenv_str_i(struct env_set *es, const char *name, const char *value, const int i)//按"%s=%s"格式将后三个参数添加到参数一所含链表的头部
{
    struct gc_arena gc = gc_new();//返回一个已初始化好的gc_arena临时变量 用于给外部变量赋值 没有分配内存
    const char *name_str = setenv_format_indexed_name(name, i, &gc);//以name_i的格式添加 并返回起始地址
    setenv_str(es, name_str, value);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
    gc_free(&gc);//释放空间
}
