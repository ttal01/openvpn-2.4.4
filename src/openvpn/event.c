...
we_init(int *maxevents, unsigned int flags)
{
    ...
    ALLOC_OBJ_CLEAR(wes, struct we_set);//分配空间并置空
    ...
    ASSERT(*maxevents > 0);
    wes->capacity = min_int(*maxevents * 2, WSA_MAXIMUM_WAIT_EVENTS);//返回参数中的最小值
    *maxevents = min_int(*maxevents, WSA_MAXIMUM_WAIT_EVENTS);

    /* Allocate space for Win32 event handles */
    ALLOC_ARRAY_CLEAR(wes->events, HANDLE, wes->capacity);//分配空间并置空
...
static void
ep_reset(struct event_set *es)//参数->fast成员需为1
{
...
static void
ep_ctl(struct event_set *es, event_t event, unsigned int rwflags, void *arg)//调用EPOLL_CTL_MOD模式 修改事件可读可写监测状态
{
    struct ep_set *eps = (struct ep_set *) es;
    struct epoll_event ev;
...
ep_wait(struct event_set *es, const struct timeval *tv, struct event_set_return *out, int outlen)
{
    ...
    if (stat > 0)
    {
        ...
        for (i = 0; i < stat; ++i)//将触发的事件拷贝到输出参数
        {
            esr->rwflags = 0;
...
ep_init(int *maxevents, unsigned int flags)
{
    ...
    ALLOC_OBJ_CLEAR(eps, struct ep_set);//分配空间并置空
...
po_init(int *maxevents, unsigned int flags)
{
    ...
    ALLOC_OBJ_CLEAR(pos, struct po_set);//分配空间并置空
...
se_init(int *maxevents, unsigned int flags)
{
    ...
    ALLOC_OBJ_CLEAR(ses, struct se_set);//分配空间并置空
...
static struct event_set *
event_set_init_simple(int *maxevents, unsigned int flags)//设定事件调用函数(（)select poll win下。。)
{
...
static struct event_set *
event_set_init_scalable(int *maxevents, unsigned int flags)//设定事件调用函数(epoll select poll win下。。)
{
    struct event_set *ret = NULL;
#if EPOLL
    ret = ep_init(maxevents, flags);//设定事件调用函数(epoll)
    if (!ret)
    {
...
#else  /* if EPOLL */
    ret = event_set_init_simple(maxevents, flags);//设定事件调用函数(select poll win下。。)
#endif
    ASSERT(ret);
    return ret;
}
...
struct event_set *
event_set_init(int *maxevents, unsigned int flags)//设定事件调用函数(epoll select poll win下。。)
{
    if (flags & EVENT_METHOD_FAST)
    {
        return event_set_init_simple(maxevents, flags);//设定事件调用函数(（)select poll win下。。)
    }
    else
    {
        return event_set_init_scalable(maxevents, flags);//设定事件调用函数(epoll select poll win下。。)
    }
}
