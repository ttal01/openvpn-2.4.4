 ...
 * horizon seconds. 一旦触发了一个动作，在horizon秒内，interval_test将保持正确。
 */
static inline void
...
static inline void
event_timeout_clear(struct event_timeout *et)//初始化参数成员
{
    et->defined = false;
    et->n = 0;
    et->last = 0;
...
static inline void
event_timeout_init(struct event_timeout *et, interval_t n, const time_t local_now)//使用参数二三初始化参数一
{
    et->defined = true;
    et->n = (n >= 0) ? n : 0;
    et->last = local_now;
...
static inline void
event_timeout_reset(struct event_timeout *et)//设定最后一次的时间
{
    if (et->defined)
    {
        et->last = now;
