...
/* actual address of remote, based on source address of received packets 远程的实际地址，基于接收数据包的源地址*/
 ...
 * in this case IP packets embedded in a TCP stream.用于在流中提取封装到缓冲区中的数据包，在这种情况下，IP数据包嵌入到TCP流中。
 */
struct stream_buf
{
 ...
 * using sockets on Posix vs. Win32.这是OpenVPN所使用的主要套接字结构。SOCKET_定义试图抽象出在Posix与Win32之间使用套接字之间的实现差异。
 */
struct link_socket
{
...
print_openvpn_sockaddr_ex(const struct openvpn_sockaddr *addr,
                          ...
                          struct gc_arena *gc)
{
    return print_sockaddr_ex(&addr->addr.sa, separator, flags, gc);//根据套接字地址获取主机及服务名 按格式保存到局部缓冲区中 并返回地址
}
...
print_openvpn_sockaddr(const struct openvpn_sockaddr *addr,
                       struct gc_arena *gc)
{
    return print_sockaddr_ex(&addr->addr.sa, ":", PS_SHOW_PORT, gc);//根据套接字地址获取主机及服务名 按格式保存到局部缓冲区中 并返回地址
}
...
print_sockaddr(const struct sockaddr *addr,
               struct gc_arena *gc)
{
    return print_sockaddr_ex(addr, ":", PS_SHOW_PORT, gc);//根据套接字地址获取主机及服务名 按格式保存到局部缓冲区中 并返回地址
}
...
static inline bool
link_socket_proto_connection_oriented(int proto)//判断是否为udp协议 是返回0
{
    return !proto_is_dgram(proto);//判断是否为udp协议
}
...
link_socket_connection_oriented(const struct link_socket *sock)
{
    if (sock)
    {
        return link_socket_proto_connection_oriented(sock->info.proto);//判断是否为udp协议 是返回0
    }
    else
    {
        return false;
...
static inline bool
addr_defined(const struct openvpn_sockaddr *addr)//地址是否定义 是则返回1
{
    if (!addr)
    {
        return 0;
...
static inline bool
link_socket_actual_defined(const struct link_socket_actual *act)//地址是否定义 是则返回1
{
...
static inline int
af_addr_size(sa_family_t af)//返回套接字大小
{
    switch (af)
    {
...
{
    if (link_socket_connection_oriented(sock))//判断是否为udp协议 是返回0
    {
...
/* read a TCP or UDP packet from link 从链接读取TCP或UDP数据包*/
static inline int
...
#else
        res = link_socket_read_udp_posix(sock, buf, from);//从套接字中读取数据到参数二
#endif
        return res;
    }
...
link_socket_write_udp(struct link_socket *sock,
                      struct buffer *buf,
                      struct link_socket_actual *to)//写udp数据
{
#ifdef _WIN32
    return link_socket_write_win32(sock, buf, to);
#else
    return link_socket_write_udp_posix(sock, buf, to);//调用sendto
#endif
}
...
link_socket_write(struct link_socket *sock,
                  struct buffer *buf,
                  struct link_socket_actual *to)//写TCP or UDP数据
{
    if (proto_is_udp(sock->info.proto)) /* unified UDPv4 and UDPv6 */
    {
        return link_socket_write_udp(sock, buf, to);//写udp数据
    }
    else if (proto_is_tcp(sock->info.proto)) /* unified TCPv4 and TCPv6 */
    {
        return link_socket_write_tcp(sock, buf, to);//写tcp数据
    }
    else
    {
        ASSERT(0);
