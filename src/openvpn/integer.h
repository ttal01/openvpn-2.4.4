...
static inline int
constrain_int(int x, int min, int max)//当参数一在后两个参数之间时返回参数一
{
    if (min > max)
    {
        return min;
 ...
 */
static inline int
modulo_add(int x, int y, int mod)//通过取模等运算返回数组下标
{
    int sum = x + y;
...
adjust_power_of_2(size_t u)//返回大于u的2的最小次幂
{
    size_t ret = 1;
