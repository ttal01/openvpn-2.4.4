...
#ifndef MANAGE_H
#define MANAGE_H
#include "config.h"     //--liut 20171110
...
/*
 * Manage log file history 管理日志文件历史
 */
...
/*
 * Management object, split into three components:管理对象分为三个部分
 *
 * struct man_persist : Data elements which are persistent across
 *                      man_connection open and close. 在man_connection中持久地打开和关闭的数据元素。
 *
 * struct man_settings : management parameters. 管理参数。
 *
 * struct man_connection : created on socket binding and listen,
 *                         deleted on socket unbind, may
 *                         handle multiple sequential client
 *                         connections. 在绑定的套接字上创建，在socket unbind上删除，可以处理多个顺序客户端连接。
 */
...
#define MS_LISTEN           1  /* no client is connected */ //监听状态
#define MS_CC_WAIT_READ     2  /* client is connected, waiting for read on socket */ //待读状态
#define MS_CC_WAIT_WRITE    3  /* client is connected, waiting for ability to write to socket */ //待写状态

struct man_connection {
    int state;

    socket_descriptor_t sd_top;//监听描述符
    socket_descriptor_t sd_cli;//???已连接的客户端？只有一个吗？
...
static inline bool
management_connected(const struct management *man)//客户端是连接的 等待读或者写状态返回1
{
