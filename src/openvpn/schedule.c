...
#ifdef ENABLE_DEBUG
static void
schedule_entry_debug_info(const char *caller, const struct schedule_entry *e)//输出计划条目调试信息
{
    ...
    if (e)
    {
        dmsg(D_SCHEDULER, "SCHEDULE: %s wakeup=[%s] pri=%u",
             caller,
             tv_string_abs(&e->tv, &gc),    //返回一个ascii字符串，描述在结构时间内的绝对日期/时间
             e->pri);
    }
    else
    {
...
/*
 * Find the earliest event to be scheduled 发现最早的事件要安排
 */
struct schedule_entry *
schedule_find_least(struct schedule_entry *e)//返回最早的事件
{
    if (e)
    {
        while (e->lt)   //迭代到最后一个条目
        {
#ifdef SCHEDULE_TEST
            ++z.lsteps;
#endif
    ...
    if (check_debug_level(D_SCHEDULER))
    {
        schedule_entry_debug_info("schedule_find_least", e);//输出计划条目调试信息
    }
#endif
...
schedule_init(void)
{
    ...
    ALLOC_OBJ_CLEAR(s, struct schedule);//分配空间并置空
    return s;
}
...
{
    return schedule_find_least(s->root);//返回最早的事件
}
...
schedule_verify(struct schedule *s)
{
    ...
    e = schedule_find_earliest_wakeup(s);//返回最早的事件
...
schedule_test(void)
{
    ...
    for (i = 0; i < n; ++i)
    {
        ALLOC_OBJ_CLEAR(array[i], struct schedule_entry);//分配空间并置空
    ...
    while ((e = schedule_find_earliest_wakeup(s)))//返回最早的事件
    {
        schedule_remove_node(s, e);
        /*schedule_verify (s);*/
    }
