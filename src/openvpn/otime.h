...
#else
    update_now(time(NULL));		//过滤因系统时钟问题导致的不稳定的时间
#endif
}
...
static inline int
openvpn_gettimeofday(struct timeval *tv, void *tz)//获取当前时间
{
...
static inline bool
tv_defined(const struct timeval *tv)//参数的秒及微妙均>0 返回1
{
...
static inline int
tv_subtract(const struct timeval *tv1, const struct timeval *tv2, const unsigned int max_seconds)//返回参数二 - 参数一
{
    ...
    return constrain_int(sec_diff * 1000000 + (tv1->tv_usec - tv2->tv_usec), -max_usec, max_usec);//当参数一在后两个参数之间时返回参数一
}
