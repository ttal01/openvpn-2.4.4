 ...
 */
void
init_options(struct options *o, const bool init_gc)//options选项初始化
{
    CLEAR(*o);
    if (init_gc)
    {
...
void
uninit_options(struct options *o)//释放空间
{
    if (o->gc_owned)
    {
        gc_free(&o->gc);//释放空间
...
setenv_connection_entry(struct env_set *es,
                        ...
                        const int i)//将连接信息按"%s=%s"格式添加到参数一所含链表的头部
{
    setenv_str_i(es, "proto", proto2ascii(e->proto, e->af, false), i);
    setenv_str_i(es, "local", e->local, i);//按"%s=%s"格式将后三个参数添加到参数一所含链表的头部
    setenv_str_i(es, "local_port", e->local_port, i);
    setenv_str_i(es, "remote", e->remote, i);
    setenv_str_i(es, "remote_port", e->remote_port, i);//按"%s=%s"格式将后三个参数添加到参数一所含链表的头部
...
void
setenv_settings(struct env_set *es, const struct options *o)//将配置、连接信息按"%s=%s"格式添加到参数一所含链表中
{
    setenv_str(es, "config", o->config);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
    ...
    if (o->connection_list)
    {
        ...
        {
            setenv_connection_entry(es, o->connection_list->array[i], i+1);//将连接信息按"%s=%s"格式添加到参数一所含链表的头部
        }
    }
    else
    {
        setenv_connection_entry(es, &o->ce, 1);//将连接信息按"%s=%s"格式添加到参数一所含链表的头部
    }
}
...
#ifndef ENABLE_SMALL
static void
show_connection_entry(const struct connection_entry *o)//输出connection_entry各类信息
{
...
static void
show_connection_entries(const struct options *o)//输出连接结构体成员的各类信息
{
    if (o->connection_list)
    {
        ...
        for (i = 0; i < l->len; ++i)
        {
            ...
            show_connection_entry(l->array[i]);//输出connection_entry各类信息
        }
    }
    else
    {
...
void
show_settings(const struct options *o) //输出各种类型信息
{
#ifndef ENABLE_SMALL
    msg(D_SHOW_PARMS, "Current Parameter Settings:");

    SHOW_STR(config);//输出字符串类型信息

    SHOW_INT(mode);//输出int类型信息
    ...
    show_connection_entries(o);//输出连接结构体成员的各类信息

    SHOW_BOOL(remote_random);//输出布尔类型信息
...
alloc_connection_entry(struct options *options, const int msglevel)//创建新条目 并插入链表options->connection_list中
{
    struct connection_list *l = alloc_connection_list_if_undef(options);//检查是否存在 返回options->connection_list
    ...
    ALLOC_OBJ_GC(e, struct connection_entry, &options->gc);//分配空间 创建并关联垃圾回收条目 并返回数据区偏移地址
    l->array[l->len++] = e;//插入链表中 options->connection_list
    return e;
}
...
static void
connection_entry_load_re(struct connection_entry *ce, const struct remote_entry *re)//将参数二信息有选择的赋给参数一
{
    if (re->remote)
    {
        ce->remote = re->remote;
...
//选项后处理 核查connection_entry成员配置是否正确
static void
options_postprocess_verify_ce(const struct options *options, const struct connection_entry *ce)
{
    ...
    init_options(&defaults, true);//defaults选项初始化
     ...
     */
    dev = dev_type_enum(options->dev, options->dev_type);//根据设备名称确定通道类型
    ...
    if (proto_is_net(ce->proto)     //判断协议是否定义
        && string_defined_equal(ce->local, ce->remote)  //字符串比较 相同返回1
...
#undef MUST_BE_UNDEF
    ...
    uninit_options(&defaults);//释放空间
}

static void
options_postprocess_mutate_ce(struct options *o, struct connection_entry *ce)//选项ce（参数二）的后处理
{
    const int dev = dev_type_enum(o->dev, o->dev_type);//根据设备名称确定通道类型
...
{
#ifdef _WIN32
    const int dev = dev_type_enum(options->dev, options->dev_type);//根据设备名称确定通道类型
#endif
...
static void
options_postprocess_verify(const struct options *o)//选项后处理 核查connection_entry成员配置是否正确
{
    if (o->connection_list)
    {
        int i;
...
static void
options_postprocess_mutate(struct options *o)//选项结构体成员的后处理
{
    int i;
    /*
     ...
     */
    helper_client_server(o);//处理服务器、桥、客户端指令 这些指令已被解析到选项结构体中
    helper_keepalive(o);//设置keepalive 10 60其它相关配置选项
    ...
    if (o->remote_list && !o->connection_list)
    {
        ...
        for (i = 0; i < rl->len; ++i)
        {
            ...
            ASSERT(re->remote);
            connection_entry_load_re(&ce, re);//将参数二信息有选择的赋给参数一
            ace = alloc_connection_entry(o, M_USAGE);//创建新条目 并插入链表o->connection_list中
            ASSERT(ace);
            *ace = ce;//将o->remote_list->array[i]的connection_entry赋值到o->connection_list->array[i]中
        }
    }
    ...
    for (i = 0; i < o->connection_list->len; ++i)
    {
        options_postprocess_mutate_ce(o, o->connection_list->array[i]);//选项ce（参数二）的后处理
    }
...
check_file_access(const int type, const char *file, const int mode, const char *opt)
{
    ...
    if (type & CHKACC_DIRPATH)
    {
        ...
        char *dirpath = dirname(fullpath);//库函数 获取文件路径

        if (platform_access(dirpath, mode|X_OK) != 0)//检查程序对文件所在目录的权限
        {
            errcode = errno;
        }
        free(fullpath);
    ...
    if (!errcode && (type & CHKACC_FILE) && (platform_access(file, mode) != 0) )//检查程序对文件本身的权限
    {
        errcode = errno;
    }

    /* If the file exists and is accessible, is it writable? */
    if (!errcode && (type & CHKACC_FILEXSTWR) && (platform_access(file, F_OK) == 0) )//检查文件本身是否存在
    {
    ...
    {
        platform_stat_t st;
        if (platform_stat(file, &st))//获取文件信息 存入参数二指向的内存中
        {
...
check_file_access_chroot(const char *chroot, const int type, const char *file, const int mode, const char *opt)
{
    ...
    if (chroot)
    {
        ...
        ret = check_file_access(type, BSTR(&chroot_file), mode, opt);//检查文件是否具备各种访问权限
        gc_free(&gc);
    }
    else
    {
        /* No chroot in play, just call core file check function */
        ret = check_file_access(type, file, mode, opt);//检查文件是否具备各种访问权限
    }
    return ret;
}
 ...
 */
static void
options_postprocess_filechecks(struct options *options)//检查文件是否具备各种访问权限 正常返回0
{
    bool errs = false;

#ifdef ENABLE_CRYPTO
    /* ** SSL/TLS/crypto related files ** */  //检查程序涉及到的各个文件是否具备各种访问权限
 ...
 */
void
options_postprocess(struct options *options)//选项结构体成员的后处理、核查及各文件权限确认
{
    options_postprocess_mutate(options);//选项结构体成员的后处理
    options_postprocess_verify(options);//选项后处理 核查connection_entry成员配置是否正确
#ifndef ENABLE_SMALL
    options_postprocess_filechecks(options);//检查文件是否具备各种访问权限 正常返回0
#endif /* !ENABLE_SMALL */
}
...
void
show_library_versions(const unsigned int flags) //打印版本信息
{
#ifdef ENABLE_CRYPTO
...
static void
usage_version(void)//打印版本相关信息
{
    msg(M_INFO|M_NOPREFIX, "%s", title_string);
    show_library_versions( M_INFO|M_NOPREFIX ); //打印版本信息
#ifdef _WIN32
...
void
notnull(const char *arg, const char *description)   //检查第一个参数 为空则输出提示信息
{
    if (!arg)
    {
...
bool
string_defined_equal(const char *s1, const char *s2)//字符串比较 相同返回1
{
    if (s1 && s2)
    {
        return !strcmp(s1, s2);
...
static inline bool
space(unsigned char c)  //判断 是否为空字符或空白字符
{
    return c == '\0' || isspace(c);
}

int
parse_line(const char *line,    //命令字符串偏移
           char *p[],       //输出缓冲区 二级指针
           const int n,     //输出缓冲区大小
           const char *file,    //配置文件名称
           const int line_num,      //行号
           int msglevel,        //日志级别
           struct gc_arena *gc)     //垃圾回收器 //解析配置文件中的每一行 返回参数个数
{
    ...
    do
    {
        ...
        {
            backslash = true;   //判断 出现了反斜线
        }
        else
        {
            if (state == STATE_INITIAL)
            {
                if (!space(in))     //判断 不为空字符或空白字符
                {
                    if (in == ';' || in == '#') /* comment 注释*/
                    {
                        break;
                    }
                    if (!backslash && in == '\"')   //双引号参数
                    {
                        state = STATE_READING_QUOTED_PARM;
                    }
                    else if (!backslash && in == '\'')  //单引号参数
                    {
                    ...
                    {
                        out = in;
                        state = STATE_READING_UNQUOTED_PARM;    //无引号参数
                    }
                }
            }
            else if (state == STATE_READING_UNQUOTED_PARM)  //无引号参数 如：proto udp
            {
                if (!backslash && space(in))    //为空白字符或空字符
                {
                    state = STATE_DONE;     //结束状态
                }
                else
                {
                    out = in;
            ...
            {
                /* ASSERT (parm_len > 0); */
                p[ret] = gc_malloc(parm_len + 1, true, gc);     //分配空间 创建垃圾回收条目 并返回数据区偏移地址
                memcpy(p[ret], parm, parm_len);     //复制到输出缓冲区
                p[ret][parm_len] = '\0';
                state = STATE_INITIAL;      //重置状态
                parm_len = 0;
                ++ret;      //如：proto udp 则按照两个参数存储
            }

            if (backslash && out)   //转义字符判断
            {
        ...
        if (out)
        {
            if (parm_len >= SIZE(parm))     //单个命令超出数组大小 打印信息并返回
            {
                parm[SIZE(parm) - 1] = 0;
            ...
            parm[parm_len++] = out;     //将从命令中读取的字符复制到局部数组中
        }
...
static void
bypass_doubledash(char **p)     //去除命令中可能存在的"--"
{
                 ...
                 struct env_set *es)
{
    ...
    if (level <= max_recursive_levels)
    {
        ...
        {
            fp = platform_fopen(file, "r");     //调用fopen()
        }
        if (fp)
        {
            line_num = 0;
            while (fgets(line, sizeof(line), fp))
            {
                ...
                if (parse_line(line + offset, p, SIZE(p)-1, file, line_num, msglevel, &options->gc))
                    //解析配置文件中的每一行 返回参数个数
                {
                    bypass_doubledash(&p[0]);       //去除命令中可能存在的"--"
    ...
    secure_memzero(line, sizeof(line)); //置空
    CLEAR(p);
}
...
parse_argv(struct options *options,
           ...
           const unsigned int permission_mask,
           unsigned int *option_types_found,
           struct env_set *es)//逐个解析命令行所有命令选项 并设置
{
    int i, j;
    ...
    if (argc == 2 && strncmp(argv[1], "--", 2))
    {
        ...
        p[0] = "config";
        p[1] = argv[1];
        add_option(options, p, NULL, 0, 0, msglevel, permission_mask, option_types_found, es);//根据配置文件设置选项
    }
    else
    {
        /* parse command line */
        for (i = 1; i < argc; ++i)  //逐个解析命令行所有命令选项
        {
            char *p[MAX_PARMS];
            CLEAR(p);
            p[0] = argv[i];
            ...
            for (j = 1; j < MAX_PARMS; ++j)//循环寻找该条命令的后续参数
            {
                if (i + j < argc)
                {
                    char *arg = argv[i + j];
...
add_option(struct options *options,
           ...
           const unsigned int permission_mask,
           unsigned int *option_types_found,
           struct env_set *es)//根据输入缓冲区命令信息设置选项
{
    struct gc_arena gc = gc_new();	//返回一个已初始化好的临时变量 用于给外部变量赋值 没有分配内存
    const bool pull_mode = BOOL_CAST(permission_mask & OPT_P_PULL_MODE);
    int msglevel_fc = msglevel_forward_compatible(options, msglevel);   //兼容
    ...
    {
        struct buffer string = alloc_buf_gc(OPTION_PARM_SIZE, &gc);//返回局部缓冲区变量 内含已注册关联的堆空间信息
        int j;
        bool good = true;
