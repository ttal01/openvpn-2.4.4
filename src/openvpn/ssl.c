...
/*
 * Forget private key password AND auth-user-pass username/password.忘记私钥密码和用户名/密码
 */
void
...
key_state_init(struct tls_session *session, struct key_state *ks)
{
    ...
    ALLOC_OBJ_CLEAR(ks->key_src, struct key_source2);//分配空间并置空

    /* allocate reliability objects */
    ALLOC_OBJ_CLEAR(ks->send_reliable, struct reliable);//分配空间并置空
    ALLOC_OBJ_CLEAR(ks->rec_reliable, struct reliable);//分配空间并置空
    ALLOC_OBJ_CLEAR(ks->rec_ack, struct reliable_ack);//分配空间并置空
...
tls_multi_init(struct tls_options *tls_options)
{
    ...
    ALLOC_OBJ_CLEAR(ret, struct tls_multi);//分配空间并置空
...
/*
 * Called by the top-level event loop.由顶级事件循环调用
 *
 * Basically decides if we should call tls_process for
 * the active or untrusted sessions.基本上决定了我们是否应该调用active或不可信会话的tls_process。
 */
...
tls_pre_decrypt(struct tls_multi *multi,
                ...
                bool floated,
                const uint8_t **ad_start)//TLS下数据包控制包解密
{
    struct gc_arena gc = gc_new();
    bool ret = false;
    ...
    if (buf->len > 0)
    {
        ...
        /* get opcode and key ID 获取操作码（高5位）和key ID（低3位）*/
        {
            uint8_t c = *BPTR(buf);
            op = c >> P_OPCODE_SHIFT;
        ...
        if ((op == P_DATA_V1) || (op == P_DATA_V2))//数据通道包
        {
            /* data channel packet */
            for (i = 0; i < KEY_SCAN_SIZE; ++i)
            {
                 ...
                 * instance and its remote peer.这是本地OpenVPN实例与远程对等点之间TLS状态兼容性的基本测试。
                 *
                 ...
                 * OpenVPN instance has no memory of such a negotiation.如果测试失败，它会告诉我们，我们收到了
                 * 来自一个消息源的数据包，该信息源声称引用了先前协商过的TLS会话，但是本地的OpenVPN实例对这样的协商没有记忆。
                 ...
                 * active side is the client which initiates connections).几乎总是发生在UDP会话中，
                 * 当连接的被动端在没有主动端重新启动的情况下重新启动(被动端是只侦听连接的服务器，活动端是启动连接的客户端)。
                 */
                    ...
                    && (floated || link_socket_actual_match(from, &ks->remote_addr)))
                {
                    ...
                    /* return appropriate data channel decrypt key in opt 返回适当的数据通道解密密钥*/
        ...
        else                      /* control channel packet 控制通道包*/
        {
            bool do_burst = false;
            bool new_link = false;
            struct session_id sid; /* remote session ID */

            /* verify legal opcode 验证合法操作码*/
            ...
            /* use session ID to match up packet with appropriate tls_session object 使用会话ID将包与适当的tls_session对象匹配*/
            for (i = 0; i < TM_SIZE; ++i)
            {
                struct tls_session *session = &multi->session[i];
                struct key_state *ks = &session->key[KS_PRIMARY];
            ...
            /*
             * Initial packet received.收到的初始数据包
             */
            ...
            if (i == TM_SIZE && is_hard_reset(op, 0))
            {
                 ...
                 * 如果我们目前没有会话，初始包将在TM_ACTIVE中打开一个新的会话，而不是TM_UNTRUSTED。
                 */
            ...
            if (i == TM_SIZE && is_hard_reset(op, 0))
            {
                 ...
                 * probably a new session. 没有与现有会话的匹配，可能是一个新的会话。
                 */
                struct tls_session *session = &multi->session[TM_UNTRUSTED];

                /*
                 * If --single-session, don't allow any hard-reset connection request
                 * unless it the the first packet of the session.如果——单会话，不允许任何重修连接请求，除非它是会话的第一个包。
                 */
                if (multi->opt.single_session)
                {
                    msg(D_TLS_ERRORS,
                 ...
                 * 在这一点上，新的session -启动控制包被验证，假设使用的是- tls- auth命令行选项。
                 * Without --tls-auth, we leave authentication entirely up to TLS.没有--tls-auth,我们将身份验证完全交给TLS。
                 */
                msg(D_TLS_DEBUG_LOW,
            ...
            else
            {
                ...
                /*
                 * Packet must belong to an existing session.  包必须属于一个现有的会话。
                 */
                ...
                /*
                 * Verify remote IP address 验证远程IP地址
                 */
                ...
                /*
                 * Remote is requesting a key renegotiation 远程请求重新协商
                 */
                ...
                {
                    /*
                     * Remote responding to our key renegotiation request? 远程回应我们的关键重新协商请求?
                     */
                    if (op == P_CONTROL_SOFT_RESET_V1)
                    {
                        do_burst = true;
            ...
            /*
             * We have an authenticated packet (if --tls-auth was set).我们有一个经过身份验证的包(如果设置了tls - auth)。
             * Now pass to our reliability level which deals with
             * packet acknowledgements, retransmits, sequencing, etc.现在通过我们的可靠性水平来处理数据包确认，重传，测序等。
             */
            {
                struct tls_session *session = &multi->session[i];
                struct key_state *ks = &session->key[KS_PRIMARY];

                /* Make sure we were initialized and that we're not in an error state 确保我们已经被初始化，并且我们没有处于错误状态*/
                ASSERT(ks->state != S_UNDEF);
                ASSERT(ks->state != S_ERROR);
                ASSERT(session_id_defined(&session->session_id));

                /* Let our caller know we processed a control channel packet 让我们的调用者知道我们处理了一个控制通道包*/
                ret = true;

                /*
                 * Set our remote address and remote session_id 设置我们的远程地址和远程session_id
                 */
                if (new_link)
                {
                 ...
                 * 我们是否应该在发送缓冲区中重新传输所有未确认的数据包?这提高了第二次对等网络上线后初始关键谈判的启动效率。
                 */
                 ...
                 * delete from reliable send buffer 处理输入的数据包，我们现在可以从可靠的发送缓冲区中删除
                 */
                {
                 ...
                 int size)
{
    ...
    else
    {
        ...
        buffer_list_push_data(ks->paybuf, data, (size_t)size);//将参数三大小的参数二拷贝到新条目空间中 并插入到参数一尾部
        ret = true;
    }
