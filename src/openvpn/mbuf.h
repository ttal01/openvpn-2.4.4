...
#define MBUF_INDEX(head, offset, size) (((head) + (offset)) & ((size)-1))   //size应该是ffff类似，求偏移后的下标位置
...
static inline bool
mbuf_defined(const struct mbuf_set *ms)//参数存在且成员长度不为0
{
    return ms && ms->len;
}
