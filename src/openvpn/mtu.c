...
alloc_buf_sock_tun(struct buffer *buf,
                   ...
                   const unsigned int align_mask)//分配空间
{
    /* allocate buffer for overlapped I/O */
    *buf = alloc_buf(BUF_SIZE(frame));//分配堆空间并置空
    ASSERT(buf_init(buf, FRAME_HEADROOM_ADJ(frame, align_mask)));//初始化
 ...
 */
void
frame_set_mtu_dynamic(struct frame *frame, int mtu, unsigned int flags)//动态设置tun MTU
{
