...
multi_tcp_init(int maxevents, int *maxclients)
{
    ...
    ALLOC_OBJ_CLEAR(mtcp, struct multi_tcp);//分配空间并置空
               ...
               struct multi_tcp *mtcp)
{
    ...
    if (management)
    {
        management_socket_set(management, mtcp->es, MTCP_MANAGEMENT, &mtcp->management_persist_flags);//调用EPOLL_CTL_MOD模式 修改事件类型
    }
#endif
...
multi_tcp_dispatch(struct multi_context *m, struct multi_instance *mi, const int action)
{
    ...
    switch (action)
    {
        ...
        case TA_SOCKET_READ_RESIDUAL:
            ...
            ASSERT(mi->context.c2.link_socket);
            set_prefix(mi);
            read_incoming_link(&mi->context);//设置为recvfrom调用读取发送到我们的TCP/UDP端口的数据报。
            clear_prefix();
            if (!IS_SIG(&mi->context))
            {
                multi_process_incoming_link(m, mi, mpp_flags);//在TCP/UDP套接字中处理数据包->TUN/ TAP接口方向
                if (!IS_SIG(&mi->context))
                {
