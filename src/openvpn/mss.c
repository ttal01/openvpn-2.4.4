 ...
 *              if yes, hand to mss_fixup_dowork()IPv4包:查找TCP报头，如果是，请检查“SYN”的标志，并将其传递给mss_fixup_dowork()
 */
void
 ...
 */ //改变SYN/SYN-ACK包的TCP MSS选项,如果存在这对IPv4和IPv6是通用的,作为TCP报头是相同的
...
mss_fixup_dowork(struct buffer *buf, uint16_t maxmss)
{
    ...
    verify_align_4(buf);//验证指针是否正确对齐
    tc = (struct openvpn_tcphdr *) BPTR(buf);
    hlen = OPENVPN_TCPH_GET_DOFF(tc->doff_res);

    /* Invalid header length or header without options. 无效的头长度或没有选项的头*/
