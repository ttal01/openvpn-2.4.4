...
static void
man_help(void)//帮助信息 列出可以使用的管理命令
{
...
static void
man_welcome(struct management *man)//打印管理端欢迎信息
{
...
static void
man_update_io_state(struct management *man)//更新连接读写状态
{
    if (socket_defined(man->connection.sd_cli))//参数已定义 返回1
    {
        if (buffer_list_defined(man->connection.out))//参数不为空 且有头条目 返回1
        {
            man->connection.state = MS_CC_WAIT_WRITE;//设为待写状态
        }
        else
        {
            man->connection.state = MS_CC_WAIT_READ;//设为待读状态
        }
    }
}

static void
man_output_list_push_finalize(struct management *man)//更新连接状态 在主事件循环之外执行管理套接字输出
{
    if (management_connected(man))//客户端是连接的 等待读或者写状态返回1
    {
        man_update_io_state(man);//更新连接读写状态
        if (!man->persist.standalone_disabled)
        {
            volatile int signal_received = 0;
            man_output_standalone(man, &signal_received);//在主事件循环之外执行管理套接字输出
        }
    }
}

static void
man_output_list_push_str(struct management *man, const char *str)//将参数二拷贝到新条目空间中 并插入到参数一->connection.out尾部
{
    if (management_connected(man) && str)//客户端是连接的 等待读或者写状态返回1
    {
        buffer_list_push(man->connection.out, (const unsigned char *) str);//将参数二拷贝到新条目空间中 并插入到参数一尾部
    }
}

static void
man_output_list_push(struct management *man, const char *str)//将参数二拷贝到参数一成员中 更新连接状态 在主事件循环之外执行管理套接字输出
{
    man_output_list_push_str(man, str);//将参数二拷贝到新条目空间中 并插入到参数一->connection.out尾部
    man_output_list_push_finalize(man);//更新连接状态 在主事件循环之外执行管理套接字输出
}

static void
man_prompt(struct management *man)//将参数二拷贝到参数一成员中 更新连接状态 在主事件循环之外执行管理套接字输出
{
    if (man_password_needed(man))
    {
        man_output_list_push(man, "ENTER PASSWORD:");//将参数二拷贝到参数一成员中 更新连接状态 在主事件循环之外执行管理套接字输出
    }
...
static void
man_delete_unix_socket(struct management *man)//删除unix路径指定的硬链接或文件
{
#if UNIX_SOCK_SUPPORT
    if ((man->settings.flags & (MF_UNIX_SOCK|MF_CONNECT_AS_CLIENT)) == MF_UNIX_SOCK)
    {
        socket_delete_unix(&man->settings.local_unix);//删除unix路径指定的硬链接或文件
    }
#endif
}

static void
man_close_socket(struct management *man, const socket_descriptor_t sd)//close(参数二)
{
#ifndef _WIN32
    /*
...
#define AF_DID_RESET (1<<1)
    ...
    if (!recursive_level) /* don't allow recursion 不允许递归*/
    {
        struct gc_arena gc = gc_new();//返回一个已初始化好的gc_arena临时变量 用于给外部变量赋值 没有分配内存
        struct log_entry e;
        const char *out = NULL;
        ...
        if (flags != M_CLIENT)
        {
            log_history_add(man->persist.log, &e);//将参数二中字符串成员复制一份到参数一的相应位置
        }
        ...
        if (!man_password_needed(man))
        {
            if (flags == M_CLIENT)
            {
                out = log_entry_print(&e, LOG_PRINT_CRLF, &gc);//将打印信息输出到缓冲区
            }
            else if (man->connection.log_realtime)
            {
                out = log_entry_print(&e, LOG_PRINT_INT_DATE
                                      ...
                                      |   LOG_PRINT_CRLF, &gc);//将打印信息输出到缓冲区
            }
            if (out)
            {
                man_output_list_push_str(man, out);//将参数二拷贝到新条目空间中 并插入到参数一->connection.out尾部
                action_flags |= AF_DID_PUSH;
            }
            if (flags & M_FATAL)
            {
                out = log_entry_print(&e, LOG_FATAL_NOTIFY|LOG_PRINT_CRLF, &gc);//将打印信息输出到缓冲区
                if (out)
                {
                    man_output_list_push_str(man, out);//将参数二拷贝到新条目空间中 并插入到参数一->connection.out尾部
        ...
        {
            man_reset_client_socket(man, true);//重置管理客户端套接字 根据参数二有两种处理
        }
 ...
 * or -1 if the signal should be ignored.给定一个信号,返回的信号可能重新映射,或-1如果信号应该被忽略。
 */
static int
man_mod_signal(const struct management *man, const int signum)//根据参数一,返回可能重新映射的参数二信号,或-1 如果信号应该被忽略。
{
...
static void
man_signal(struct management *man, const char *name)//解析参数二信号 进行重新映射
{
    const int sig = parse_signal(name);//解析信号名字 返回对应的信号值
    if (sig >= 0)
    {
        const int sig_mod = man_mod_signal(man, sig);//根据参数一,返回可能重新映射的参数二信号,或-1 如果信号应该被忽略
        if (sig_mod >= 0)
        {
            throw_signal(sig_mod);//抛出信号的全局设置
            msg(M_CLIENT, "SUCCESS: signal %s thrown", signal_name(sig_mod, true));//返回信号的大写或小写名字
        }
        else
        {
            if (man->persist.special_state_msg)
...
static void
in_extra_reset(struct man_connection *mc, const int mode)//根据参数二处置参数一的成员空间
{
    if (mc)
    {
        if (mode != IER_NEW)
        ...
        if (mc->in_extra)
        {
            buffer_list_free(mc->in_extra);//释放参数及成员空间
            mc->in_extra = NULL;
        }
        if (mode == IER_NEW)
        {
            mc->in_extra = buffer_list_new(0);//分配空间并设置参数的值给成员
        }
    }
}

static void
in_extra_dispatch(struct management *man)//??相应命令分派处理
{
    switch (man->connection.in_extra_cmd)
    {
        ...
        case IEC_CLIENT_AUTH:
            if (man->persist.callback.client_auth)
            {
                const bool status = (*man->persist.callback.client_auth)
                                        ...
                                        NULL,
                                        NULL,
                                        man->connection.in_extra);//对应管理命令 client-auth
        ...
        case IEC_CLIENT_PF:
            if (man->persist.callback.client_pf)
            {
                const bool status = (*man->persist.callback.client_pf)
                                        (man->persist.callback.arg,
                                        man->connection.in_extra_cid,
                                        man->connection.in_extra);//对应管理命令 client-pf
                man->connection.in_extra = NULL;
                if (status)
                {
        ...
        case IEC_RSA_SIGN:
            ...
            buffer_list_free(man->connection.ext_key_input);//释放参数及成员空间
        ...
        case IEC_CERTIFICATE:
            ...
            buffer_list_free(man->connection.ext_cert_input);//释放参数及成员空间
            man->connection.ext_cert_input = man->connection.in_extra;
            man->connection.in_extra = NULL;
            return;
#endif
    }
    in_extra_reset(&man->connection, IER_RESET);//根据参数二处置参数一的成员空间
}
...
static void
man_env_filter(struct management *man, const int level)//根据参数二设置参数一成员值
{
...
static bool
man_need(struct management *man, const char **p, const int n, unsigned int flags)//判断参数二所含的参数三个成员不为空
{
    int i;
    ASSERT(p[0]);
    for (i = 1; i <= n; ++i)
...
static void
man_dispatch_command(struct management *man, struct status_output *so, const char **p, const int nparms)//分派指令并执行
{
    struct gc_arena gc = gc_new();
    ...
    else if (streq(p[0], "help")) // !strcmp() 相等返回1
    {
        man_help();//帮助信息 列出可以使用的管理命令
    }
    else if (streq(p[0], "version")) //打印版本信息
    {
        msg(M_CLIENT, "OpenVPN Version: %s", title_string);
        msg(M_CLIENT, "Management Version: %d", MANAGEMENT_VERSION);
        msg(M_CLIENT, "END");
    }
    else if (streq(p[0], "pid")) //打印进程id
    {
        msg(M_CLIENT, "SUCCESS: pid=%d", platform_getpid());//获取进程号
    }
#ifdef MANAGEMENT_DEF_AUTH
    ...
    else if (streq(p[0], "env-filter"))
    {
        ...
        man_env_filter(man, level);//根据参数二设置参数一成员值
    }
#endif
    else if (streq(p[0], "signal"))
    {
        if (man_need(man, p, 1, 0))//判断参数二所含的参数三个成员不为空
        {
            man_signal(man, p[1]);//解析参数二信号 进行重新映射
        }
    }
#ifdef TARGET_ANDROID
...
static void
man_connection_settings_reset(struct management *man)//重设连接参数
{
...
man_new_connection_post(struct management *man, const char *description)
{
    ...
    set_nonblock(man->connection.sd_cli);//设置非阻塞

    man_connection_settings_reset(man);//重设连接参数
    ...
    if (man->settings.flags & MF_UNIX_SOCK)
    {
        msg(D_MANAGEMENT, "MANAGEMENT: %s %s",
            description,
            sockaddr_unix_name(&man->settings.local_unix, "NULL"));//返回绑定的unix地址路径
    }
    else
#endif
    msg(D_MANAGEMENT, "MANAGEMENT: %s %s",
        description,
        print_sockaddr(man->settings.local->ai_addr, &gc));//根据套接字地址获取主机及服务名 按格式保存到局部缓冲区中 并返回地址

    buffer_list_reset(man->connection.out);//释放参数指向的所有链表缓冲区空间 并置空指针

    if (!man_password_needed(man))
    {
        man_welcome(man);//打印管理端欢迎信息
    }
    man_prompt(man);//将参数二拷贝到参数一成员中 更新连接状态 在主事件循环之外执行管理套接字输出
...
#if UNIX_SOCK_SUPPORT
static bool
man_verify_unix_peer_uid_gid(struct management *man, const socket_descriptor_t sd)//核查uid gid
{
    if (socket_defined(sd) && (man->settings.client_uid != -1 || man->settings.client_gid != -1))
    {
        ...
        int uid, gid;
        if (unix_socket_get_peer_uid_gid(man->connection.sd_cli, &uid, &gid))//获取uid gid
        {
...
static void
man_accept(struct management *man)//accept接受连接请求 设置文件描述符fork时关闭
{
    struct link_socket_actual act;
    CLEAR(act);

    /*
     * Accept the TCP or Unix domain socket client.接受TCP或Unix域套接字客户端
     */
#if UNIX_SOCK_SUPPORT
    if (man->settings.flags & MF_UNIX_SOCK)
    {
        struct sockaddr_un remote;
        man->connection.sd_cli = socket_accept_unix(man->connection.sd_top, &remote);//接收unix连接请求 设置文件描述符fork时关闭 返回连接描述符
        if (!man_verify_unix_peer_uid_gid(man, man->connection.sd_cli))//核查uid gid
        {
            sd_close(&man->connection.sd_cli);//关闭套接字
        }
    }
    else
#endif
    man->connection.sd_cli = socket_do_accept(man->connection.sd_top, &act, false);//接收inet连接请求 设置文件描述符fork时关闭 返回连接描述符
...
static void
man_listen(struct management *man)//创建套接字 绑定地址 设置监听
{
    struct gc_arena gc = gc_new();
    ...
    /*
     * Initialize listening socket 初始化监听套接字
     */
    if (man->connection.sd_top == SOCKET_UNDEFINED)
    {
        ...
        if (man->settings.flags & MF_UNIX_SOCK)
        {
            man_delete_unix_socket(man);//先删除unix路径指定的硬链接或文件
            man->connection.sd_top = create_socket_unix();//创建并返回监听套接字
            socket_bind_unix(man->connection.sd_top, &man->settings.local_unix, "MANAGEMENT");//更改权限以绑定套接字地址
        }
        else
#endif
        {
            man->connection.sd_top = create_socket_tcp(man->settings.local);//创建tcp套接字 端口复用
            socket_bind(man->connection.sd_top, man->settings.local,
                        man->settings.local->ai_family, "MANAGEMENT", false);//绑定套接字地址
        }

        /*
         * Listen for connection
         */
        if (listen(man->connection.sd_top, 1))//转换为监听套接字
        {
         ...
         */
        set_nonblock(man->connection.sd_top);//设置非阻塞
...
static void
man_connect(struct management *man)//创建套接字 连接客户端？ 内部有IO循环？
{
    struct gc_arena gc = gc_new();
    int status;
    ...
    if (man->settings.flags & MF_UNIX_SOCK)
    {
        man->connection.sd_cli = create_socket_unix();//创建并返回套接字
        status = socket_connect_unix(man->connection.sd_cli, &man->settings.local_unix);//连接connect()
        if (!status && !man_verify_unix_peer_uid_gid(man, man->connection.sd_cli))//核查uid gid
        {
#ifdef EPERM
            status = EPERM;
#else
...
#endif
    {
        man->connection.sd_cli = create_socket_tcp(man->settings.local);//创建tcp套接字 端口复用
        status = openvpn_connect(man->connection.sd_cli,
                                 man->settings.local->ai_addr,
                                 5,
                                 &signal_received);//连接connect()
    }

    if (signal_received)
    {
        throw_signal(signal_received);//抛出信号的全局设置
        goto done;
    }
...
static void
man_reset_client_socket(struct management *man, const bool exiting)//重置管理客户端套接字 根据参数二有两种处理
{
    if (socket_defined(man->connection.sd_cli))//参数已定义 返回1
    {
#ifdef _WIN32
        man_stop_ne32(man);
#endif
        man_close_socket(man, man->connection.sd_cli);//close(参数二)
        man->connection.sd_cli = SOCKET_UNDEFINED;
        man->connection.state = MS_INITIAL;
        command_line_reset(man->connection.in);//置空并初始化参数成员
        buffer_list_reset(man->connection.out);//释放参数指向的所有链表缓冲区空间 并置空指针
#ifdef MANAGEMENT_IN_EXTRA
        in_extra_reset(&man->connection, IER_RESET);//根据参数二处置参数一的成员空间
#endif
    ...
    if (!exiting)
    {
        ...
        {
            ssl_purge_auth(false);//忘记私钥密码和用户名/密码
        }
#endif
        if (man->settings.flags & MF_SIGNAL)
        {
            int mysig = man_mod_signal(man, SIGUSR1);//根据参数一,返回参数二信号可能重新映射,或-1如果信号应该被忽略
            if (mysig >= 0)
            {
                msg(D_MANAGEMENT, "MANAGEMENT: Triggering management signal");
                throw_signal_soft(mysig, "management-disconnect");//信号软中断
            }
        }
        ...
        if (man->settings.flags & MF_CONNECT_AS_CLIENT)
        {
            ...
            throw_signal_soft(SIGTERM, "management-exit");//信号软中断
        }
        else
        {
            man_listen(man);//创建套接字 绑定地址 设置监听
        }
    }
}

static void
man_process_command(struct management *man, const char *line)//管理命令处理
{
    struct gc_arena gc = gc_new();
    ...
    CLEAR(parms);
    so = status_open(NULL, 0, -1, &man->persist.vout, 0);//分配空间 赋值并返回
#ifdef MANAGEMENT_IN_EXTRA
    in_extra_reset(&man->connection, IER_RESET);//根据参数二处置参数一的成员空间
#endif
    ...
    {
        nparms = parse_line(line, parms, MAX_PARMS, "TCP", 0, M_CLIENT, &gc);//解析配置文件中的每一行 返回参数个数
        ...
        if (nparms > 0)
        {
            man_dispatch_command(man, so, (const char **)parms, nparms);//分派指令并执行
        }
    }

    CLEAR(parms);//置空
    status_close(so);//关闭文件 释放参数 出错返回0
    gc_free(&gc);
}

static bool
man_io_error(struct management *man, const char *prefix)//如果这是一个不可忽略的系统错误 返回1
{
    const int err = openvpn_errno();

    if (!ignore_sys_error(err))//如果这是一个可以安全忽略的系统错误，则返回true
    {
        struct gc_arena gc = gc_new();
...
static int
man_read(struct management *man)//读取套接字 执行读取的命令？
{
    /*
    ...
    len = recv(man->connection.sd_cli, buf, sizeof(buf), MSG_NOSIGNAL);//读取套接字
#endif

    if (len == 0)
    {
        man_reset_client_socket(man, false);//重置管理客户端套接字 根据参数二 软中断或重新监听
    }
    else if (len > 0)
    {
        ...
        command_line_add(man->connection.in, buf, len);//将参数二拷贝到参数一成员空间中

        /*
         * Reset output object
         */
        buffer_list_reset(man->connection.out);//释放参数指向的所有链表缓冲区空间 并置空成员指针
         ...
         */
        {
            ...
            while ((line = command_line_get(man->connection.in)))//取出参数缓冲区成员中的最后一行数据
            {
#ifdef MANAGEMENT_IN_EXTRA
                if (man->connection.in_extra)
                {
                    ...
                    {
                        in_extra_dispatch(man);//??相应命令分派处理
                    }
                    else
                    {
                        buffer_list_push(man->connection.in_extra, line);//将参数二拷贝到新条目空间中 并插入到参数一尾部
                    }
                }
                else
#endif
                man_process_command(man, (char *) line);//管理命令处理
                if (man->connection.halt)
                {
                    break;
                }
                command_line_next(man->connection.in);//参数两个成员之间进行拷贝
    ...
    {
        if (man_io_error(man, "recv"))//如果这是一个不可忽略的系统错误 返回1
        {
            man_reset_client_socket(man, false);//重置管理客户端套接字 根据参数二 软中断或重新监听
        }
    }
...
static int
man_write(struct management *man)//发送数据 清除发送的数据内存
{
    ...
    buf = buffer_list_peek(man->connection.out);//返回参数成员的头节点缓冲区
    if (buf && BLEN(buf))
    {
        const int len = min_int(size_hint, BLEN(buf));//返回参数中的最小值
#ifdef TARGET_ANDROID
...
#endif
        sent = send(man->connection.sd_cli, BPTR(buf), len, MSG_NOSIGNAL);//发送数据
        if (sent >= 0)
        {
            buffer_list_advance(man->connection.out, sent);//弹出链表头节点
        }
        else if (sent < 0)
        {
            if (man_io_error(man, "send"))//如果这是一个不可忽略的系统错误 返回1
            {
                man_reset_client_socket(man, false);//重置管理客户端套接字 根据参数二 软中断或重新监听
            }
        }
...
man_persist_init(struct management *man,
                 ...
                 const int state_buffer_size)//man_persist成员初始化
{
    ...
    if (!mp->defined)
    {
        ...
        /* initialize log history store 初始化日志历史存储*/
        mp->log = log_history_init(log_history_cache);		//创建变量，并保存分配的空间

        /*
         * Initialize virtual output object, so that functions
         * which write to a virtual_output object can be redirected
         * here to the management object.初始化虚拟输出对象，以便将写入到virtual_output对象的函数重新定向到管理对象。
         */
        mp->vout.func = virtual_output_callback_func;	//???
        mp->vout.arg = man;
        mp->vout.flags_default = M_CLIENT;
        msg_set_virtual_output(&mp->vout);		//赋值给全局const指针

        /*
         * Initialize --echo list
         */
        man->persist.echo = log_history_init(echo_buffer_size);		//创建变量，并保存分配的空间

        /*
         * Initialize --state list
         */
        man->persist.state = log_history_init(state_buffer_size);	//创建变量，并保存分配的空间
...
static void
man_connection_init(struct management *man)//设定事件处理函数 监听/连接套接字
{
    if (man->connection.state == MS_INITIAL)
    {
         ...
         * command output from/to the socket.为命令行输入和命令输出分配helper对象从/到套接字。
         */
         ...
         * running outside of the primary event loop.当我们在主事件循环之外运行时，初始化事件集作为独立使用。
         */
        {
            int maxevents = 1;
            man->connection.es = event_set_init(&maxevents, EVENT_METHOD_FAST);//设定事件调用函数(epoll select poll win下。。)
        }
        ...
        if (man->settings.flags & MF_CONNECT_AS_CLIENT)
        {
            man_connect(man);//创建套接字 连接客户端？ 内部有IO循环？
        }
        else
        {
            man_listen(man);//创建套接字 绑定地址 设置监听
...
struct management *
management_init(void)		//管理对象初始化
{
    struct management *man;
    ALLOC_OBJ_CLEAR(man, struct management);	//分配空间，创建管理对象 并置空

    man_persist_init(man,
                     MANAGEMENT_LOG_HISTORY_INITIAL_SIZE,
                     MANAGEMENT_ECHO_BUFFER_SIZE,
                     MANAGEMENT_STATE_BUFFER_SIZE);		//初始化管理对象的数据成员

    man_connection_clear(&man->connection);		//置空 连接成员 并初始化赋值
...
management_open(struct management *man,
                ...
                const char *write_peer_info_file,
                const int remap_sigusr1,
                const unsigned int flags)//含 监听/连接套接字
{
    bool ret = false;
    ...
    if (man->connection.state == MS_INITIAL)
    {
        ...
        {
            man_connection_init(man);//设定事件处理函数 监听/连接套接字
            ret = true;
        }
    }
                     ...
                     const struct openvpn_sockaddr *remote)
{
    if (man->persist.state && (!(man->settings.flags & MF_SERVER) || state < OPENVPN_STATE_CLIENT_BASE))
    {
        ...
        log_history_add(man->persist.state, &e);//将参数二中字符串成员复制一份到参数一的相应位置
...
management_echo(struct management *man, const char *string, const bool pull)
{
    if (man->persist.echo)
    {
        ...
        log_history_add(man->persist.echo, &e);//将参数二中字符串成员复制一份到参数一的相应位置
...
void
management_post_tunnel_open(struct management *man, const in_addr_t tun_local_ip)//监听/连接套接字
{
    /*
...
static inline bool
man_persist_state(unsigned int *persistent, const int n)//如果*参数二等于参数一 返回0 否则 赋值并返回1 参数一为空 返回1
{
    if (persistent)
    {
...
management_io(struct management *man)
{
    if (man->connection.state != MS_INITIAL)
    {
        ...
        {
            man_reset_client_socket(man, false);//重置管理客户端套接字 根据参数二有两种处理
        }
        else
        {
...
management_socket_set(struct management *man,
                      struct event_set *es,
                      void *arg,
                      unsigned int *persistent)//调用EPOLL_CTL_MOD模式 修改事件类型
{
    switch (man->connection.state)
    {
        case MS_LISTEN:
            if (man_persist_state(persistent, 1))//如果*参数二等于参数一 返回0 否则 赋值并返回1 参数一为空 返回1
            {
                event_ctl(es, man->connection.sd_top, EVENT_READ, arg);//调用EPOLL_CTL_MOD模式 修改事件为EPOLLIN
            }
            break;
        ...
        case MS_CC_WAIT_READ:
            if (man_persist_state(persistent, 2))
            {
                event_ctl(es, man->connection.sd_cli, EVENT_READ, arg);//调用EPOLL_CTL_MOD模式 修改事件为EPOLLIN
            }
            break;
        ...
        case MS_CC_WAIT_WRITE:
            if (man_persist_state(persistent, 3))
            {
                event_ctl(es, man->connection.sd_cli, EVENT_WRITE, arg);//调用EPOLL_CTL_MOD模式 修改事件为EPOLLOUT
            }
            break;
...
void
management_io(struct management *man)//管理io accept() read() write()
{
    switch (man->connection.state)
    {
        case MS_LISTEN:
            man_accept(man);//accept接受连接请求 设置文件描述符fork时关闭
            break;

        case MS_CC_WAIT_READ:
            man_read(man);//读取套接字 执行读取的命令？
            break;

        case MS_CC_WAIT_WRITE:
            man_write(man);//发送数据 清除发送的数据内存
            break;
...
static bool
man_check_for_signals(volatile int *signal_received)//获取收到的信号 存入参数中 有信号 返回1
{
    if (signal_received)
    {
        get_signal(signal_received);//获取收到的信号 存入参数中
        if (*signal_received)
        {
            return true;
        }
...
/*
 * Wait for socket I/O when outside primary event loop 在主事件循环之外等待socket I/O
 */
static int
man_block(struct management *man, volatile int *signal_received, const time_t expire)//在主事件循环之外等待socket I/O(信号、epoll事件)
{
    struct timeval tv;
    ...
    if (man_standalone_ok(man))
    {
        while (true)
        {
            event_reset(man->connection.es);//参数->fast成员需为1
            management_socket_set(man, man->connection.es, NULL, NULL);//调用EPOLL_CTL_MOD模式 修改事件类型
            tv.tv_usec = 0;
            tv.tv_sec = 1;
            if (man_check_for_signals(signal_received))//获取收到的信号 存入参数中 有信号 返回1
            {
                status = -1;
                break;
            }
            status = event_wait(man->connection.es, &tv, &esr, 1);//等待1个事件发生 如epoll_wait()
            update_time();//更新当前时间
            if (man_check_for_signals(signal_received))//获取收到的信号 存入参数中 有信号 返回1
            {
                status = -1;
            ...
            {
                /* set SIGINT signal if expiration time exceeded 设置SIGINT信号，如果过期时间已超过*/
                status = 0;
                if (signal_received)
                {
...
/*
 * Perform management socket output outside primary event loop 在主事件循环之外执行管理套接字输出
 */
static void
man_output_standalone(struct management *man, volatile int *signal_received)
{
    if (man_standalone_ok(man))
    {
        while (man->connection.state == MS_CC_WAIT_WRITE)//待写状态
        {
            management_io(man);//管理io accept() read() write()
            if (man->connection.state == MS_CC_WAIT_WRITE)
            {
                man_block(man, signal_received, 0);//在主事件循环之外等待socket I/O(信号、epoll事件)
            }
...
man_standalone_event_loop(struct management *man, volatile int *signal_received, const time_t expire)
{
    ...
    if (man_standalone_ok(man))
    {
        status = man_block(man, signal_received, expire);//在主事件循环之外等待socket I/O(信号、epoll事件)
        if (status > 0)
        {
            management_io(man);//管理io accept() read() write()
        }
...
command_line_new(const int buf_len)
{
    ...
    ALLOC_OBJ_CLEAR(cl, struct command_line);//分配空间并置空
...
void
command_line_reset(struct command_line *cl)//置空并初始化参数成员
{
    buf_clear(&cl->buf);//置空并初始化参数成员
...
void
command_line_add(struct command_line *cl, const unsigned char *buf, const int len)//将参数二拷贝到参数一成员空间中
{
    int i;
    for (i = 0; i < len; ++i)
    {
        if (buf[i] && char_class(buf[i], (CC_PRINT|CC_NEWLINE)))//根据字符类型对字符串进行筛选
        {
            if (!buf_write_u8(&cl->buf, buf[i]))//添加数据到参数一
            {
                buf_clear(&cl->buf);//置空并初始化参数成员
            }
        }
...
const unsigned char *
command_line_get(struct command_line *cl)//取出参数缓冲区成员中的最后一行数据
{
    int i;
    ...
    if (i >= 0)
    {
        buf_copy_excess(&cl->residual, &cl->buf, i);//将参数二截断，将多余的数据复制到参数一
        buf_chomp(&cl->buf);//删除尾部回车换行字符，确保空终止
        ret = (const unsigned char *) BSTR(&cl->buf);//返回当前缓冲区的内部上一条数据结尾偏移地址（即当前数据开始地址）
    }
    return ret;
}

void
command_line_next(struct command_line *cl)//参数两个成员之间进行拷贝
{
    buf_clear(&cl->buf);//置空并初始化参数成员
    buf_copy(&cl->buf, &cl->residual);//拷贝参数二到参数一
    buf_clear(&cl->residual);//置空并初始化参数成员
}
...
const char *
log_entry_print(const struct log_entry *e, unsigned int flags, struct gc_arena *gc)//将打印信息输出到缓冲区
{
    struct buffer out = alloc_buf_gc(ERR_BUF_SIZE, gc);//返回局部缓冲区变量 内含已注册关联的堆空间信息
    if (flags & LOG_FATAL_NOTIFY)
    {
        buf_printf(&out, ">FATAL:");//格式化输出（附加）到缓冲区中
    }
...
static void
log_entry_free_contents(struct log_entry *e)//释放参数成员并置空参数
{
    if (e->string)
    {
        free((char *)e->string);
...
static inline int
log_index(const struct log_history *h, int i)//下标+1 通过取模等运算返回数组下标
{
    return modulo_add(h->base, i, h->capacity);//通过取模等运算返回数组下标
}

static void
log_history_obj_init(struct log_history *h, int capacity)//置空并初始化赋值，含开辟空间
{
    CLEAR(*h);
    h->capacity = capacity;
    ALLOC_ARRAY_CLEAR(h->array, struct log_entry, capacity);	//分配参数二*参数三大小空间，并检查是否超限，并置空
}

struct log_history *
log_history_init(const int capacity)//创建变量，并保存分配的空间
{
    struct log_history *h;
    ASSERT(capacity > 0);
    ALLOC_OBJ(h, struct log_history);		//创建变量
    log_history_obj_init(h, capacity);		//置空并初始化赋值，含开辟空间
    return h;
}
...
void
log_history_add(struct log_history *h, const struct log_entry *le)//将参数二中字符串成员复制一份到参数一的相应位置
{
    struct log_entry *e;
    ASSERT(h->size >= 0 && h->size <= h->capacity);
    if (h->size == h->capacity)//到达数组尾部
    {
        e = &h->array[h->base];//指向有效数据起始位置
        log_entry_free_contents(e);//释放参数成员并置空参数???不会访问非法内存吗==》不会，因为上一行代码已经赋值了
        h->base = log_index(h, 1);//有效数据起始下标+1 通过取模等运算返回数组下标
    }
    else
    {
        e = &h->array[log_index(h, h->size)];//本轮次数据中 指向有效数据末尾位置
        ++h->size;//递增有效数据个数
    }

    *e = *le;//赋值
    e->string = string_alloc(le->string, NULL);//拷贝参数一到新开辟的堆空间 返回堆空间地址
}
...
log_history_resize(struct log_history *h, const int capacity)
{
    if (capacity != h->capacity)
    {
        ...
        for (i = 0; i < h->size; ++i)
        {
            log_history_add(&newlog, &h->array[log_index(h, i)]);//将参数二中字符串成员复制一份到参数一的相应位置
        }
