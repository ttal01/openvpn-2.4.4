                     ...
                     const struct mroute_addr *addr)
{
    ...
    if (plugin_defined(plugins, OPENVPN_PLUGIN_LEARN_ADDRESS))
    {
        ...
        argv_reset(&argv);//释放空间 并初始化参数
    }

    if (m->top.options.learn_address_script)
    {
        struct argv argv = argv_new();
        setenv_str(es, "script_type", "learn-address");//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
        argv_parse_cmd(&argv, m->top.options.learn_address_script);//解析配置文件 添加到参数一中
...
/*
 * Main initialization function, init multi_context object.主要初始化函数,初始化multi_context对象。
 */
void
multi_init(struct multi_context *m, struct context *t, bool tcp_mode, int thread_mode)
{
     ...
     */
    dev = dev_type_enum(t->options.dev, t->options.dev_type);//根据设备名称确定通道类型

    /*
     * Init our multi_context object.初始化multi_context对象。
     */
    CLEAR(*m);
     ...
     */ //真实地址哈希表(源端口号被认为是地址的一部分)。用于确定哪个客户端发送了一个从TCP/UDP套接字上看到的传入数据包
     ...
     * which client to route a packet to.虚拟地址的哈希表。用于确定将数据包路由到哪个客户端。
     */
     ...
     */ //这个哈希表是m->hash 的克隆，但是有一个bucket大小的哈希表，这样它就可以在列表中快速迭代。
    m->iter = hash_init(1,
                        get_random(),
                        mroute_addr_hash_function,
                        mroute_addr_compare_function);
    ...
    /*
     * Allocate broadcast/multicast buffer list 分配广播/多播缓冲区列表
     */
    m->mbuf = mbuf_init(t->options.n_bcast_buf);

    /*
     * Different status file format options are available 有不同的状态文件格式选项
     */
     ...
     * tunnel. 可能分配一个ifconfig池，以不同的方式，以一个tun或tap风格的隧道。
     */
    if (t->options.ifconfig_pool_defined)
    {
        ...
        /* reload pool data from file 从文件中重新加载池数据*/
    ...
    /*
     * Help us keep track of routing table.帮助我们跟踪路由表。
     */
    m->route_helper = mroute_helper_init(MULTI_CACHE_ROUTE_TTL);

    /*
     * Initialize route and instance reaper. 初始化路由和实例收割机。
     */
    m->reaper = multi_reap_new(reap_buckets_per_pass(t->options.virtual_hash_size));

    /*
     * Get local ifconfig address 获取本地ifconfig地址
     */
    CLEAR(m->local);
    ASSERT(t->c1.tuntap);
    ...
    /*
     * Initialize multi-socket TCP I/O wait object 初始化多 TCP I/O等待对象
     */
    if (tcp_mode)
    {
     ...
     * tun/tap interface and network stack? 允许客户端< - >客户端通信，不通过tun / tap接口和网络堆栈?
     */
    m->enable_c2c = t->options.enable_c2c;

    /* initialize stale routes check timer 初始化陈旧的路由检查计时器*/
...
multi_create_instance(struct multi_context *m, const struct mroute_addr *real)
{
    ...
    ALLOC_OBJ_CLEAR(mi, struct multi_instance);//分配空间并置空
...
/*
 * Learn a virtual address or route.学习虚拟地址或路由。
 * The learn will fail if the learn address
 * script/plugin fails.  In this case the
 * return value may be != mi.如果学习地址脚本/插件失败，学习将失败。在这种情况下，返回值可能是!= mi。
 * Return the instance which owns this route,
 * or NULL if none.返回拥有此路由的实例，如果没有，则为NULL。
 */
static struct multi_instance *
multi_learn_addr(struct multi_context *m,
                 struct multi_instance *mi,
                 const struct mroute_addr *addr,
                 const unsigned int flags)//学习虚拟地址或路由 返回拥有此路由的实例，如果没有，则为NULL。
{
    struct hash_element *he;
    ...
    /* if route currently exists, get the instance which owns it 如果路由当前存在，则获取拥有它的实例*/
        ...
        && !mroute_addr_equal(addr, &m->local))
    {
        ...
        /* The cache is invalidated when cache_generation is incremented 当cache_generation增加时，缓存无效*/
...
/*
 * Get client instance based on virtual address.基于虚拟地址获取客户端实例。
 */
static struct multi_instance *
                                   ...
                                   bool cidr_routing)
{
    ...
    /* check for local address 检查本地地址*/
    if (mroute_addr_equal(addr, &m->local))//比较两个参数 相同返回1
    {
        return NULL;
    }

    route = (struct multi_route *) hash_lookup(m->vhash, addr);//返回已存在的并移动为头节点的->value成员

    /* does host route (possible cached) exist? 主机路由(可能缓存)是否存在?*/
    ...
    else if (cidr_routing) /* do we need to regenerate a host route cache entry? 我们需要重新生成一个主机路由缓存条目吗?*/
    {
        struct mroute_helper *rh = m->route_helper;
        struct mroute_addr tryaddr;
        int i;

        /* cycle through each CIDR length 循环通过每一个CIDR长度*/
        for (i = 0; i < rh->n_net_len; ++i)
        {
            ...
            /* look up a possible route with netbits netmask 使用netbits netmask查找可能的路由*/
            route = (struct multi_route *) hash_lookup(m->vhash, &tryaddr);//返回已存在的节点的->value成员

            if (route && multi_route_defined(m, route))
            {
                /* found an applicable route, cache host route 找到一条适用的路线，缓存主机路由*/
                struct multi_instance *mi = route->instance;
                multi_learn_addr(m, mi, addr, MULTI_ROUTE_CACHE|MULTI_ROUTE_AGEABLE);//学习虚拟地址或路由 返回拥有此路由的实例，如果没有，则为NULL。
                ret = mi;
                break;
            }
        }
...
/*
 * Select a virtual address for a new client instance.为新客户端实例选择一个虚拟地址。
 * Use an --ifconfig-push directive, if given (static IP).使用- ifconfig- push指令，如果给定(静态IP)。
 * Otherwise use an --ifconfig-pool address (dynamic IP).使用- ifconfig- pool地址(动态IP)。
 */
static void
multi_select_virtual_addr(struct multi_context *m, struct multi_instance *mi)
{
     ...
     * release pool addresses, otherwise keep them.如果ifconfig地址由动态配置文件设置，则释放池地址，否则保留它们。
     */
    if (mi->context.options.push_ifconfig_defined)
    {
        /* ifconfig addresses were set statically,
         * release dynamic allocation      //ifconfig地址被静态设置，释放动态分配*/
         ...
         * 当前的实现不允许“静态IPv4，池IPv6”(见下文)，因此，如果出现这种情况，就发出警告——不要中断会话，因为我们甚至不知道这个客户是否需要IPv6
         */
    ...
    else if (m->ifconfig_pool && mi->vaddr_handle < 0) /* otherwise, choose a pool address 否则，选择一个池地址*/
    {
...
/*
 * Called to load management-derived client-connect config调用负载管理派生的客户端连接配置
 */
static void
                         ...
                         unsigned int *option_types_found)
{
    if (config)
    {
         ...
         * with an --ifconfig-push directive, it will override any
         * --ifconfig-push directive from the --client-config-dir
         * directory or any --ifconfig-pool dynamic address.如果-客户端连接脚本生成一个配置文件，该文件带有一个-ifconfig-push指令，
         * 它将覆盖来自-客户-config-dir目录的任何-ifconfig-push指令，或者任何-ifconfig-pool动态地址。
         */
        multi_select_virtual_addr(m, mi);
        multi_set_virtual_addr_env(m, mi);
    }
...
/*
 * Called as soon as the SSL/TLS connection authenticates.当SSL / TLS连接验证时立即调用。
...
multi_connection_established(struct multi_context *m, struct multi_instance *mi)
{
    if (tls_authentication_status(mi->context.c2.tls_multi, 0) == TLS_AUTHENTICATION_SUCCEEDED)
    {
        ...
        int cc_succeeded = true; /* client connect script status 客户端连接脚本状态*/
        int cc_succeeded_count = 0;

        ASSERT(mi->context.c1.tuntap);

        /* lock down the common name and cert hashes so they can't change during future TLS renegotiations
            锁定通用名称和cert散列，以便在未来的TLS重新谈判期间不能更改*/
        tls_lock_common_name(mi->context.c2.tls_multi);
        tls_lock_cert_hash_set(mi->context.c2.tls_multi);

        /* generate a msg() prefix for this client instance 为这个客户端实例生成一个msg()前缀*/
        generate_prefix(mi);

        /* delete instances of previous clients with same common-name 删除以前具有相同名称的客户实例*/
        if (!mi->context.options.duplicate_cn)
        {
            multi_delete_dup(m, mi);
        }

        /* reset pool handle to null 重置池句柄为空*/
        mi->vaddr_handle = -1;

        /*
         * Try to source a dynamic config file from the
         * --client-config-dir directory.尝试从客户机- config- dir目录找到动态配置文件
         */
         ...
         * or --ifconfig-pool.从--ifconfig-push文件或- ifconfig- pool中选择一个虚拟地址。
         */
        multi_select_virtual_addr(m, mi);

        /* do --client-connect setenvs ——客户机连接setenvs*/
        multi_client_connect_setenv(m, mi);

#ifdef ENABLE_PLUGIN
        /*
         * Call client-connect plug-in.调用客户机连接插件
         */

        /* deprecated callback, use a file for passing back return info 不赞成的回调，使用一个文件来传递返回信息*/
...
script_depr_failed:
        ...
        /* V2 callback, use a plugin_return struct for passing back return info 回调，使用plugin_return结构来传递返回信息*/
        ...
        /*
         * Run --client-connect script.运行——客户机连接脚本。
         */
...
script_failed:
        ...
        /*
         * Check for client-connect script left by management interface client检查由管理接口客户端留下的客户端连接脚本
         */
#ifdef MANAGEMENT_DEF_AUTH
         ...
         * or config file generated by --client-connect script.在客户端- config- dir文件或由客户端- connect脚本生成的配置文件中检查“禁用”指令。
         */
        ...
        if (cc_succeeded)
        {
            ...
            /*
             * make sure we got ifconfig settings from somewhere确保我们从某个地方得到了ifconfig设置
             */
            if (!mi->context.c2.push_ifconfig_defined)
            {
            ...
            /*
             * make sure that ifconfig settings comply with constraints确保ifconfig设置符合约束条件
             */
            if (!ifconfig_push_constraint_satisfied(&mi->context))
            {
                /* JYFIXME -- this should cause the connection to fail 这将导致连接失败*/
             ...
             * plus add all iroute routes.对于路由隧道，将内部路由设置为端点，并添加所有的iroute路由。
             */
            if (TUNNEL_TYPE(mi->context.c1.tuntap) == DEV_TYPE_TUN)
            {
                if (mi->context.c2.push_ifconfig_defined)
                 ...
                 * --iroute options have been specified 在本地添加路由，指向新客户端，如果指定了iroute选项*/
                multi_add_iroutes(m, mi);

                /*
                 * iroutes represent subnets which are "owned" by a particular
                 * client.  Therefore, do not actually push a route to a client
                 * if it matches one of the client's iroutes.iroutes表示一个特定客户“拥有”的子网。因此，
                 * 如果客户端与客户端的iroutes匹配，则不要实际推动路由到客户端。
                 */
            ...
            /* set our client's VPN endpoint for status reporting purposes 将客户端VPN端点设置为状态报告目的*/
            mi->reporting_addr = mi->context.c2.push_ifconfig_local;
            mi->reporting_addr_ipv6 = mi->context.c2.push_ifconfig_ipv6_local;

            /* set context-level authentication flag 设置环境级别的认证标志*/
            mi->context.c2.context_auth = CAS_SUCCEEDED;

#ifdef ENABLE_ASYNC_PUSH
            /* authentication complete, send push reply 认证完成，发送推送回复*/
        ...
        /* set flag so we don't get called again 设置环境级别的认证标志*/
        mi->connection_established_flag = true;

        /* increment number of current authenticated clients 增加当前已认证客户端的数量*/
        ++m->n_clients;
        update_mstat_n_clients(m->n_clients);
        --mi->n_clients_delta;
    ...
    /*
     * Reply now to client's PUSH_REQUEST query现在回复客户的PUSH_REQUEST查询
     */
 ...
 * instance. 向特定实例添加mbuf缓冲区。
 */
void
multi_add_mbuf(struct multi_context *m,
               struct multi_instance *mi,
               struct mbuf_buffer *mb)//向特定实例添加mbuf缓冲区。
{
    if (multi_output_queue_ready(m, mi))//如果输出队列不满，返回true
    {
        struct mbuf_item item;
        item.buffer = mb;
        item.instance = mi;
        mbuf_add_item(m->mbuf, &item);//添加参数二到参数一成员数组尾部 引用+1
    }
    else
    {
...
/*
 * Add a packet to a client instance output queue. 向客户端实例输出队列添加一个包。
 */
static inline void
multi_unicast(struct multi_context *m,
              const struct buffer *buf,
              struct multi_instance *mi)//向客户端实例输出队列添加一个包。
{
    struct mbuf_buffer *mb;

    if (BLEN(buf) > 0)
    {
        mb = mbuf_alloc_buf(buf);//拷贝参数及其成员空间 返回副本
        mb->flags = MF_UNICAST;
        multi_add_mbuf(m, mi, mb);//向特定实例添加mbuf缓冲区。
        mbuf_free_buf(mb);//当引用值为0时 释放空间
    }
}

/*
 * Broadcast a packet to all clients.向所有客户广播一个数据包
 */
static void
multi_bcast(struct multi_context *m,
            const struct buffer *buf,
            const struct multi_instance *sender_instance,
            const struct mroute_addr *sender_addr)//向所有客户广播一个数据包
{
    struct hash_iterator hi;
    struct hash_element *he;
    struct multi_instance *mi;
 ...
 * 特定于实例的计时器，将最早的时间转换为绝对时间，调用调度器和我们未来的唤醒时间
 * Also close context on signal.信号上也要关闭上下文。
 */
bool
multi_process_post(struct multi_context *m, struct multi_instance *mi, const unsigned int flags)
{
    ...
    if (!IS_SIG(&mi->context) && ((flags & MPP_PRE_SELECT) || ((flags & MPP_CONDITIONAL_PRE_SELECT) && !ANY_OUT(&mi->context))))
    {
        ...
        pre_select(&mi->context);//定时器的设定与相关处理
        ...
        if (!IS_SIG(&mi->context))
        {
             ...
             * and (if specified) auth user/pass succeeds 当SSL/TLS密钥协商成功且(如果指定)auth用户/pass成功时，连接将“建立”*/
            if (!mi->connection_established_flag && CONNECTION_ESTABLISHED(&mi->context))
            {
                multi_connection_established(m, mi);
            }

            /* tell scheduler to wake us up at some point in the future 告诉调度程序在将来某个时候叫醒我们*/
    ...
    {
        /* continue to pend on output? 继续对输出进行pend ?*/
 ...
 * i.e. client -> server direction.在TCP/UDP套接字中处理数据包->TUN/ TAP接口方向，即。客户端- >服务器方向。
 */
bool
multi_process_incoming_link(struct multi_context *m, struct multi_instance *instance, const unsigned int mpp_flags)
{
    ...
    //这是判断目的地址的有效性，作为OpenVPN服务器来讲，从虚拟网卡中来的数据包就是要发往OpenVPN客户端的数据包，因此需要判断目的地址，
    //和multi_process_incoming_link的判断相反。
    if (!instance)
    {
#ifdef MULTI_DEBUG_EVENT_LOOP
    ...
    if (m->pending)
    {
        ...
        /* get instance context 得到实例上下文*/
        c = &m->pending->context;

        if (!instance)
        {
            /* transfer packet pointer from top-level context buffer to instance 将数据包指针从顶级上下文缓冲区转移到实例*/
            c->c2.buf = m->top.c2.buf;

            /* transfer from-addr from top-level context buffer to instance 转移from-addr从顶级上下文缓存到实例*/
            if (!floated)
            {
        ...
        if (BLEN(&c->c2.buf) > 0)
        {
            ...
            /* decrypt in instance context 在实例上下文中解密*/

            perf_push(PERF_PROC_IN_LINK);
            lsi = get_link_socket_info(c);
            orig_buf = c->c2.buf.data;
            if (process_incoming_link_part1(c, lsi, floated))//解封装输入数据包
            {
                if (floated)
                {
                ...
                process_incoming_link_part2(c, lsi, orig_buf);//解封装输入数据包
            }
            perf_pop();

            if (TUNNEL_TYPE(m->top.c1.tuntap) == DEV_TYPE_TUN)
            {
                /* extract packet source and dest addresses 提取信息包的来源和目的地址*/
                mroute_flags = mroute_extract_addr_from_packet(&src,
                                                               &dest,
                                                               NULL,
                                                               NULL,
                                                               &c->c2.to_tun,
                                                               DEV_TYPE_TUN);//在buf中给出一个原始数据包，返回src和dest地址。

                /* drop packet if extract failed 如果提取失败，则删除包*/
                if (!(mroute_flags & MROUTE_EXTRACT_SUCCEEDED))
                {
                    c->c2.to_tun.len = 0;
                }
                /* make sure that source address is associated with this client 确保源地址与此客户机关联*/
                else if (multi_get_instance_by_virtual_addr(m, &src, true) != m->pending)//基于虚拟地址获取客户端实例。
                {
                ...
                /* client-to-client communication enabled? client-to-client沟通启用吗?*/
                else if (m->enable_c2c)//如果c2c启用的话
                {
                    /* multicast? */
                    if (mroute_flags & MROUTE_EXTRACT_MCAST)//组播
                    {
                        /* for now, treat multicast as broadcast 现在，把多播作为广播*/
                        multi_bcast(m, &c->c2.to_tun, m->pending, NULL);
                    }
                    else /* possible client to client routing 可能的c2c客户端路由*/
                    {
                        ASSERT(!(mroute_flags & MROUTE_EXTRACT_BCAST));
                        mi = multi_get_instance_by_virtual_addr(m, &dest, true); //server作为“路由器”找到目的client的socket

                        /* if dest addr is a known client, route to it 如果目的addr是一个已知的客户，路由到它*/
                        if (mi)
                        {
#ifdef ENABLE_PF
...
#endif
                                                               &c->c2.to_tun,
                                                               DEV_TYPE_TAP);//在buf中给出一个原始数据包，返回src和dest地址。

                if (mroute_flags & MROUTE_EXTRACT_SUCCEEDED)
                {
                    if (multi_learn_addr(m, m->pending, &src, 0) == m->pending)//学习虚拟地址或路由 返回拥有此路由的实例，如果没有，则为NULL。
                    {
                        /* check for broadcast */
                        if (m->enable_c2c)//以下是广播/多播情况，比如tap模式下的arp就是广播，可以看出mroute_extract_addr_ether处理了arp，设置了多播标志，OpenVPN中，多播和广播统一处理
                        {
                            ...
                            else /* try client-to-client routing //下面根据目的mac来判断目的地是否是同一个虚拟子网的，如果是，则将以太帧发过去*/
                            {
                                mi = multi_get_instance_by_virtual_addr(m, &dest, false);//基于虚拟地址获取客户端实例。
...
#endif
                                    {
                                        multi_unicast(m, &c->c2.to_tun, mi);//向客户端实例输出队列添加一个包。//单播发送以太帧
                                        register_activity(c, BLEN(&c->c2.to_tun));
                                    }
        ...
        /* postprocess and set wakeup //所有其它情况都将受到的数据包写入虚拟网卡中，一旦写入虚拟网卡以后，数据包的流向就不再受OpenVPN的控制了，而是完全受内核路由表的控制*/
 ...
 * i.e. server -> client direction.处理数据包 在TUN/TAP接口->TCP/UDP套接字方向，即服务器->客户端方向。
 */
bool
multi_process_incoming_tun(struct multi_context *m, const unsigned int mpp_flags)
{
    ...
    if (BLEN(&m->top.c2.buf) > 0)
    {
         ...
         * the appropriate multi_instance object.将传入的tun/tap包路由到适当的multi_instance对象。
         */
        ...
        mroute_flags = mroute_extract_addr_from_packet(&src,
                                                       ...
                                                       NULL,
                                                       &m->top.c2.buf,
                                                       dev_type);//在buf中给出一个原始数据包，返回src和dest地址。

        if (mroute_flags & MROUTE_EXTRACT_SUCCEEDED)
        {
            struct context *c;

            /* broadcast or multicast dest addr? 广播还是多播?*/
            if (mroute_flags & (MROUTE_EXTRACT_BCAST|MROUTE_EXTRACT_MCAST))
            {
                /* for now, treat multicast as broadcast 现在，把多播作为广播*/
#ifdef ENABLE_PF
                multi_bcast(m, &m->top.c2.buf, NULL, e2);//向所有客户广播一个数据包
#else
            ...
            else
            {
                ...
                if (m->pending)
                {
                    ...
                    /* encrypt in instance context 加密在实例上下文中*/
                    process_incoming_tun(c);//封装c->c2.buf一个输出数据包到c->c2.to_link

                    /* postprocess and set wakeup 后处理,唤醒*/
                    ret = multi_process_post(m, m->pending, mpp_flags);
 ...
 * queue.在队列中处理可能的客户-客户端/ bcast/ mcast消息
 */
struct multi_instance *
multi_get_queue(struct mbuf_set *ms)//取出队列中的数据 封装后保存到c->c2.to_link
{
    struct mbuf_item item;

    if (mbuf_extract_item(ms, &item)) /* cleartext IP packet //取出原头节点数据至参数二 后移头节点下标*/
    {
        unsigned int pip_flags = PIPV4_PASSTOS;

        set_prefix(item.instance);
        item.instance->context.c2.buf = item.buffer->buf;//赋值要输出的数据（数据可能是c2c传过来的）
        if (item.buffer->flags & MF_UNICAST) /* --mssfix doesn't make sense for broadcast or multicast 对广播或多播没有意义*/
        {
            pip_flags |= PIP_MSSFIX;
        }
        process_ip_header(&item.instance->context, pip_flags, &item.instance->context.c2.buf);//检查处理ip头
        encrypt_sign(&item.instance->context, true);//压缩，片段，加密和hmac 签署一个输出数据包到c->c2.to_link。
        mbuf_free_buf(item.buffer);//当引用值为0时 释放空间
 ...
 * false if it should continue. 如果事件循环应该中断，则返回true
 */
bool
...
tunnel_server(struct context *top)
{
    ...
    if (proto_is_dgram(top->options.ce.proto))//判断是否为udp协议
    {
