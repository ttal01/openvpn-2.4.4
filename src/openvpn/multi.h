...
#ifndef MULTI_H
#define MULTI_H
#include "syshead.h" //增加头文件 --liut 171115
#if P2MP_SERVER
 ...
 *这个结构是使用在服务器模式下运行的OpenVPN进程来存储与一个VPN隧道相关的状态信息。
 ...
 * server-mode.  “VPN隧道状态存储结构”相关页面描述了OpenVPN在服务器模式下运行时结构的作用。
 */ //tun模式中，multi_instance代表三层路由目标，tap模式中，multi_instance代表二层路由目标，即端口。
...
/**
 * Main OpenVPN server state structure. 主OpenVPN服务器状态结构
 *
 * This structure is used by OpenVPN processes running in server-mode to
 * store all the VPN tunnel and process-wide state.这个结构是通过在服务器模式运行的OpenVPN进程来存储所有VPN隧道和处理器范围的状态。
 *
 * The @ref tunnel_state "Structure of VPN tunnel state storage" related
 * page describes the role the structure plays when OpenVPN is running in
 * server-mode. “VPN隧道状态存储结构”相关页面描述了OpenVPN在服务器模式下运行时结构的作用。
...
/*
 * Return true if our output queue is not full //如果输出队列不满，返回true
 */
static inline bool
 ...
 * the to_link buffer.确定哪个实例具有待定输出，并为发送到to_link缓冲区的输出做好准备。
 */
static inline struct multi_instance *
multi_process_outgoing_link_pre(struct multi_context *m)//确定哪个实例具有待定输出，并为发送到to_link缓冲区的输出做好准备。
{
    ...
    {
        mi = multi_get_queue(m->mbuf);//取出队列中的数据 封装后保存到c->c2.to_link
    }
    return mi;
}
 ...
 * all instances.  Output: 从所有实例的集合中计算最早的超时到期时间。输出:
 *
 * m->earliest_wakeup : instance needing the earliest service. 实例需要最早的服务。
 * dest               : earliest timeout as a delta in relation
 *                      to current time. 最大:与当前时间相比，最早的超时时间。
 */
static inline void
multi_get_timeout(struct multi_context *m, struct timeval *dest)//从所有实例的集合中计算最早的超时到期时间
{
    struct timeval tv, current;

    CLEAR(tv);
    m->earliest_wakeup = (struct multi_instance *) schedule_get_earliest_wakeup(m->schedule, &tv);//返回最早唤醒时间的节点
    if (m->earliest_wakeup)
    {
        ASSERT(!openvpn_gettimeofday(&current, NULL));//获取当前时间
...
multi_process_outgoing_link_dowork(struct multi_context *m, struct multi_instance *mi, const unsigned int mpp_flags)
{
    bool ret = true;
    set_prefix(mi);
    process_outgoing_link(&mi->context);//处理写数据
    ret = multi_process_post(m, mi, mpp_flags);
    clear_prefix();
    return ret;
}
