 ...
 */
static inline void
check_tls(struct context *c)//TLS会话需要服务吗
{
 ...
 */
static inline void
check_tls_errors(struct context *c)//某些情况下 TLS错误会需要重启
{
#if defined(ENABLE_CRYPTO)
 ...
 */
static inline void
check_incoming_control_channel(struct context *c)//检查控制通道上的输入配置信息
{
#if P2MP
 ...
 */
static inline void
check_connection_established(struct context *c)//检查连接的建立 触发相应选项
{
    void check_connection_established_dowork(struct context *c);//???

    if (event_timeout_defined(&c->c2.wait_for_connect))
    {
        check_connection_established_dowork(c);//连接启动后需要立即发生的事情应该到这里
    }
}
 ...
 */
static inline void
check_add_routes(struct context *c)//是否增加路由
{
 ...
 */
static inline void
check_inactivity_timeout(struct context *c)//可能由于--inactive退出
{
 ...
 */
static inline void
check_status_file(struct context *c)//打印统计信息
{
    void check_status_file_dowork(struct context *c);//打印统计信息

    if (c->c1.status_output)
    {
        if (status_trigger_tv(c->c1.status_output, &c->c2.timeval))
        {
            check_status_file_dowork(c);//打印统计信息
        }
    }
}
...
static inline void
context_reschedule_sec(struct context *c, int sec)//重订上下文时间间隔
{
    if (sec < 0)
    {
        sec = 0;
...
register_activity(struct context *c, const int size)
{
    if (c->options.inactivity_timeout)
    {
        ...
        if (c->c2.inactivity_bytes >= c->options.inactivity_minimum_bytes)
        {
            ...
            event_timeout_reset(&c->c2.inactivity_interval);//设定最后一次的时间
        }
    }
}
...
p2p_iow_flags(const struct context *c)//返回适合点对点隧道的io_wait()标记
{
 ...
 */
static inline void
io_wait(struct context *c, const unsigned int flags)//内核I/O等待函数，用于所有I/O等待，除了服务器模式的TCP
{
    void io_wait_dowork(struct context *c, const unsigned int flags);//等待I/O事件 用于点对点模式下的TCP和UDP套接字 多点模式下的UDP套接字
    ...
    {
        /* slow path */
        io_wait_dowork(c, flags);//等待I/O事件 用于点对点模式下的TCP和UDP套接字 多点模式下的UDP套接字
    }
}
