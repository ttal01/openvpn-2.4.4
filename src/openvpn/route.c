...
static bool
add_bypass_address(struct route_bypass *rb, const in_addr_t a)//参数一数组中是否有参数二 没有则添加
{
    int i;
    for (i = 0; i < rb->n_bypass; ++i)
    {
...
static bool
is_route_parm_defined(const char *parm)//参数是否定义 是则返回1
{
    if (!parm)
    {
        return false;
...
static void
setenv_route_addr(struct env_set *es, const char *key, const in_addr_t addr, int i)//设置路由环境变量
{
    struct gc_arena gc = gc_new();
    ...
    setenv_str(es, BSTR(&name), print_in_addr_t(addr, 0, &gc));//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
    gc_free(&gc);
}
...
get_special_addr(const struct route_list *rl,
                 const char *string,
                 in_addr_t *out,
                 bool *status)//判断参数二 将参数一信息赋值给参数三
{
    if (status)
    {
        *status = true;
...
init_route(struct route_ipv4 *r,
           ...
           const struct route_list *rl)//初始化路由信息
{
    ...
    if (!is_route_parm_defined(ro->network))//参数是否定义 是则返回1
    {
        goto fail;
    }
    ...
    if (get_special_addr(rl, ro->network, (in_addr_t *) &special.s_addr, &status))//判断参数二 将参数一信息赋值给参数三
    {
        special.s_addr = htonl(special.s_addr);
        ret = openvpn_getaddrinfo(0, inet_ntoa(special), NULL, 0, NULL,
                                  AF_INET, network_list);//将IPv4/IPv6 addr or hostname解析并保存到末尾参数中
    }
    else
    {
    ...
    if (is_route_parm_defined(ro->netmask))//参数是否定义 是则返回1
    {
        r->netmask = getaddr(
            GETADDR_HOST_ORDER
            ...
            0,
            &status,
            NULL);//将IPv4/IPv6 addr or hostname转为网络字节序
        if (!status)
        {
            goto fail;
        }
    ...
    if (is_route_parm_defined(ro->gateway))//参数是否定义 是则返回1
    {
        if (!get_special_addr(rl, ro->gateway, &r->gateway, &status))//判断参数二 将参数一信息赋值给参数三
        {
            r->gateway = getaddr(
                GETADDR_RESOLVE
                | GETADDR_HOST_ORDER
    ...
    r->metric = 0;
    if (is_route_parm_defined(ro->metric))//参数是否定义 是则返回1
    {
...
static void
clear_route_list(struct route_list *rl)//释放空间并置空
{
    gc_free(&rl->gc);
    CLEAR(*rl);
}
                     ...
                     in_addr_t target)
{
        ...
        && rl->rgi.gateway.netmask < 0xFFFFFFFF)
    {
        ...
        ALLOC_OBJ_GC(r1, struct route_ipv4, &rl->gc);/分配空间 创建并关联垃圾回收条目 并返回数据区偏移地址
...
add_block_local(struct route_list *rl)
{
        ...
        && rl->spec.remote_host_local != TLA_LOCAL)
    {
        ...
        add_bypass_address(&rl->spec.bypass, rl->rgi.gateway.addr);//参数一数组中是否有参数二 没有则添加
#endif
                ...
                struct env_set *es)//初始化路由链表信息
{
    ...
    get_default_gateway(&rl->rgi);//获取默认网关信息
    if (rl->rgi.flags & RGI_ADDR_DEFINED)
    {
        setenv_route_addr(es, "net_gateway", rl->rgi.gateway.addr, -1);//设置路由环境变量
    ...
    if (is_route_parm_defined(remote_endpoint))//参数是否定义 是则返回1
    {
        bool defined = false;
    ...
    /* parse the routes from opt to rl */
    {
        ...
        for (ro = opt->routes; ro; ro = ro->next)
        {
            ...
            if (!init_route(&r, &netlist, ro, rl))//初始化路由信息
            {
                ret = false;
            }
            else
            {
                ...
                for (curele = netlist; curele; curele = curele->ai_next)
                {
                    ...
                    ALLOC_OBJ_GC(new, struct route_ipv4, &rl->gc);//分配空间 创建并关联垃圾回收条目 并返回数据区偏移地址
                    *new = r;
                    new->network = ntohl(((struct sockaddr_in *)curele->ai_addr)->sin_addr.s_addr);
                    new->next = rl->routes;
                    rl->routes = new;//插入到链表头部
                }
            }
            if (netlist)
            {
...
static void
setenv_route(struct env_set *es, const struct route_ipv4 *r, int i)//设置路由环境变量
{
    struct gc_arena gc = gc_new();
    if (r->flags & RT_DEFINED)
    {
        setenv_route_addr(es, "network", r->network, i);//设置路由环境变量
        ...
        if (r->flags & RT_METRIC_DEFINED)
        {
            ...
            setenv_int(es, BSTR(&name), r->metric);//将参数二参数三数据内容按"%s=%s"添加到参数一所含链表的头部
        }
    }
    gc_free(&gc);
}
...
void
setenv_routes(struct env_set *es, const struct route_list *rl)//设置路由环境变量
{
    int i = 1;
    struct route_ipv4 *r;
    for (r = rl->routes; r; r = r->next)
    {
        setenv_route(es, r, i++);//设置路由环境变量
    }
}
...
get_windows_routing_table(struct gc_arena *gc)//获取当前默认路由表
{
    ULONG size = 0;
...
void
get_default_gateway(struct route_gateway_info *rgi)//获取默认网关信息
{
    struct gc_arena gc = gc_new();//返回一个已初始化好的gc_arena临时变量 用于给外部变量赋值 没有分配内存

    const IP_ADAPTER_INFO *adapters = get_adapter_info_list(&gc);//得到适配器信息列表
    const MIB_IPFORWARDTABLE *routes = get_windows_routing_table(&gc);//获取当前默认路由表
